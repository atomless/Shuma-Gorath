...
test-dashboard-tab-information-architecture: ## Run focused dashboard tab information-architecture contract checks
	...
	@$(MAKE) --no-print-directory test-dashboard-svelte-check
	@node --test \
		--test-name-pattern='dashboard state and store contracts remain immutable and bounded with heartbeat-owned connection telemetry|dashboard smoke spec keeps the tab information architecture aligned with the canonical registry|monitoring, traffic, and diagnostics tabs make ownership boundaries explicit' \
		e2e/dashboard.modules.unit.test.js
	@if $(MAKE) --no-print-directory spin-wait-ready; then \
		$(MAKE) --no-print-directory seed-dashboard-data || exit 1; \
		SHUMA_BASE_URL=http://127.0.0.1:3000 SHUMA_API_KEY=$(SHUMA_API_KEY) SHUMA_FORWARDED_IP_SECRET=$(SHUMA_FORWARDED_IP_SECRET) ./scripts/tests/run_dashboard_e2e.sh --grep "monitoring, traffic, and diagnostics tabs expose their ownership split"; \
	else \
		echo "$(RED)❌ Error: Spin server not ready$(NC)"; \
		echo "$(YELLOW)   Start the server first: make dev$(NC)"; \
		exit 1; \
...
test-dashboard-monitoring-accountability: ## Run focused dashboard Monitoring machine-contract accountability checks
	@echo "$(CYAN)🧪 Running focused dashboard Monitoring accountability checks...$(NC)"
	@if ! command -v corepack >/dev/null 2>&1; then \
		echo "$(RED)❌ Error: corepack not found (install Node.js 18+).$(NC)"; \
		exit 1; \
	fi
	...
	@$(MAKE) --no-print-directory test-dashboard-svelte-check
	@node --test \
		--test-name-pattern='dashboard monitoring accountability adapters normalize benchmark and oversight payloads safely|dashboard monitoring accountability refresh populates machine snapshots through behavior' \
		e2e/dashboard.modules.unit.test.js
	@if $(MAKE) --no-print-directory spin-wait-ready; then \
		$(MAKE) --no-print-directory seed-dashboard-data || exit 1; \
		SHUMA_BASE_URL=http://127.0.0.1:3000 SHUMA_API_KEY=$(SHUMA_API_KEY) SHUMA_FORWARDED_IP_SECRET=$(SHUMA_FORWARDED_IP_SECRET) ./scripts/tests/run_dashboard_e2e.sh --grep "monitoring projects benchmark and oversight accountability from machine-first contracts"; \
	else \
		echo "$(RED)❌ Error: Spin server not ready$(NC)"; \
		echo "$(YELLOW)   Start the server first: make dev$(NC)"; \
		exit 1; \
...
test-dashboard-traffic-pane: ## Run focused dashboard Traffic tab ownership and refresh checks
	...
	@$(MAKE) --no-print-directory test-dashboard-svelte-check
	@node --test \
		--test-name-pattern='dashboard smoke spec keeps the tab information architecture aligned with the canonical registry|dashboard route exposes live traffic refresh controls without changing other tab semantics|monitoring, traffic, and diagnostics tabs make ownership boundaries explicit' \
		e2e/dashboard.modules.unit.test.js
	@if $(MAKE) --no-print-directory spin-wait-ready; then \
		$(MAKE) --no-print-directory seed-dashboard-data || exit 1; \
		SHUMA_BASE_URL=http://127.0.0.1:3000 SHUMA_API_KEY=$(SHUMA_API_KEY) SHUMA_FORWARDED_IP_SECRET=$(SHUMA_FORWARDED_IP_SECRET) ./scripts/tests/run_dashboard_e2e.sh --grep "monitoring, traffic, and diagnostics tabs expose their ownership split"; \
