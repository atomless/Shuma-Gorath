...
DEV_LOCAL_CONTRIBUTOR_INGRESS_ENABLE ?= 1
...
ADVERSARY_SIM_LOCAL_RUNTIME_ENV := SHUMA_SIM_TELEMETRY_SECRET=$(SHUMA_SIM_TELEMETRY_SECRET) SHUMA_FORWARDED_IP_SECRET=$(SHUMA_FORWARDED_IP_SECRET) SHUMA_FRONTIER_OPENAI_API_KEY=$(SHUMA_FRONTIER_OPENAI_API_KEY) SHUMA_FRONTIER_ANTHROPIC_API_KEY=$(SHUMA_FRONTIER_ANTHROPIC_API_KEY) SHUMA_FRONTIER_GOOGLE_API_KEY=$(SHUMA_FRONTIER_GOOGLE_API_KEY) SHUMA_FRONTIER_XAI_API_KEY=$(SHUMA_FRONTIER_XAI_API_KEY) SHUMA_FRONTIER_OPENAI_MODEL=$(SHUMA_FRONTIER_OPENAI_MODEL) SHUMA_FRONTIER_ANTHROPIC_MODEL=$(SHUMA_FRONTIER_ANTHROPIC_MODEL) SHUMA_FRONTIER_GOOGLE_MODEL=$(SHUMA_FRONTIER_GOOGLE_MODEL) SHUMA_FRONTIER_XAI_MODEL=$(SHUMA_FRONTIER_XAI_MODEL) $(LOCAL_CONTRIBUTOR_INGRESS_ENV) ADVERSARY_SIM_SCRAPLING_SCOPE_DESCRIPTOR_PATH=$(SCRAPLING_LOCAL_SCOPE_PATH) ADVERSARY_SIM_SCRAPLING_SEED_INVENTORY_PATH=$(SCRAPLING_LOCAL_SEED_PATH) ADVERSARY_SIM_SCRAPLING_CRAWLDIR=$(SCRAPLING_LOCAL_CRAWLDIR)
...
runtime-verify-freshness: ## Fail if Rust runtime source is newer than the current prebuilt dist/wasm artifact
	@RUNTIME_STAMP="$(WASM_ARTIFACT)"; \
	if [ ! -f "$$RUNTIME_STAMP" ] || \
	   [ Cargo.toml -nt "$$RUNTIME_STAMP" ] || \
	   [ Cargo.lock -nt "$$RUNTIME_STAMP" ] || \
	   [ spin.toml -nt "$$RUNTIME_STAMP" ] || \
	   find src -type f \( -name '*.rs' -o -name '*.toml' \) -newer "$$RUNTIME_STAMP" -print -quit | grep -q .; then \
		echo "Runtime build freshness verification failed: $(WASM_ARTIFACT) is older than runtime sources."; \
		echo "Run: make build-runtime"; \
		echo "Then restart Spin (for example, make dev, make run, or make run-prebuilt) before rerunning runtime-backed e2e checks."; \
		exit 1; \
	fi
...
dev: ## Build and run with file watching (auto-rebuild on save)
	...
	@if [ "$${DEV_FORCE_REBUILD:-0}" = "1" ] || [ ! -f $(WASM_BUILD_OUTPUT) ] || [ ! -f $(WASM_ARTIFACT) ] || \
	   ...
	   find src -name "*.rs" -newer $(WASM_BUILD_OUTPUT) -print -quit | grep -q .; then \
		...
		touch $(WASM_BUILD_OUTPUT) $(WASM_ARTIFACT); \
	else \
	...
	@./scripts/dev_watch_lock.sh cargo watch --poll -w src -w dashboard -w spin.toml $(DEV_WATCH_IGNORES) \
		-s 'if [ ! -f $(WASM_BUILD_OUTPUT) ] || find src -name "*.rs" -newer $(WASM_BUILD_OUTPUT) -print -quit | grep -q .; then ./scripts/set_crate_type.sh cdylib && cargo build --target wasm32-wasip1 --release && mkdir -p $(dir $(WASM_ARTIFACT)) && cp $(WASM_BUILD_OUTPUT) $(WASM_ARTIFACT) && ./scripts/set_crate_type.sh rlib && touch $(WASM_BUILD_OUTPUT) $(WASM_ARTIFACT); else echo "No Rust changes detected; skipping WASM rebuild."; fi' \
		-s 'pkill -x spin 2>/dev/null || true; $(MAKE) --no-print-directory prepare-scrapling-local >/dev/null 2>&1 && $(MAKE) --no-print-directory config-verify && $(MAKE) --no-print-directory dashboard-build >/dev/null 2>&1 && $(LOCAL_LOOPBACK_BAN_CLEANUP_BACKGROUND) RUNTIME_INSTANCE_ID="$$(uuidgen)" && SHUMA_API_KEY=$(SHUMA_API_KEY) SHUMA_FORWARDED_IP_SECRET=$(SHUMA_FORWARDED_IP_SECRET) SHUMA_ADVERSARY_SIM_SUPERVISOR_BASE_URL=$(LOCAL_CONTRIBUTOR_PUBLIC_BASE_URL) SHUMA_ADVERSARY_SIM_AVAILABLE=$(DEV_ADVERSARY_SIM_AVAILABLE) $(LOCAL_CONTRIBUTOR_INGRESS_ENV) $(ADVERSARY_SIM_LOCAL_RUNTIME_ENV) SPIN_ALWAYS_BUILD=0 ./scripts/run_with_oversight_supervisor.sh spin up --direct-mounts $(SPIN_ENV_ONLY_BASE) $(SPIN_DEV_OVERRIDES) --env RUNTIME_INSTANCE_ID=$$RUNTIME_INSTANCE_ID --listen $(LOCAL_CONTRIBUTOR_ORIGIN_LISTEN)'
...
dev-closed: ## Build and run with file watching and SHUMA_KV_STORE_FAIL_OPEN=false (fail-closed)
	...
	@./scripts/dev_watch_lock.sh cargo watch --poll -w src -w dashboard -w spin.toml $(DEV_WATCH_IGNORES) \
		-s 'if [ ! -f $(WASM_BUILD_OUTPUT) ] || find src -name "*.rs" -newer $(WASM_BUILD_OUTPUT) -print -quit | grep -q .; then ./scripts/set_crate_type.sh cdylib && cargo build --target wasm32-wasip1 --release && mkdir -p $(dir $(WASM_ARTIFACT)) && cp $(WASM_BUILD_OUTPUT) $(WASM_ARTIFACT) && ./scripts/set_crate_type.sh rlib && touch $(WASM_BUILD_OUTPUT) $(WASM_ARTIFACT); else echo "No Rust changes detected; skipping WASM rebuild."; fi' \
		-s 'pkill -x spin 2>/dev/null || true; $(MAKE) --no-print-directory prepare-scrapling-local >/dev/null 2>&1 && $(MAKE) --no-print-directory config-verify && $(MAKE) --no-print-directory dashboard-build >/dev/null 2>&1 && $(LOCAL_LOOPBACK_BAN_CLEANUP_BACKGROUND) RUNTIME_INSTANCE_ID="$$(uuidgen)" && SHUMA_API_KEY=$(SHUMA_API_KEY) SHUMA_FORWARDED_IP_SECRET=$(SHUMA_FORWARDED_IP_SECRET) SHUMA_ADVERSARY_SIM_SUPERVISOR_BASE_URL=$(LOCAL_CONTRIBUTOR_PUBLIC_BASE_URL) SHUMA_ADVERSARY_SIM_AVAILABLE=$(DEV_ADVERSARY_SIM_AVAILABLE) $(LOCAL_CONTRIBUTOR_INGRESS_ENV) $(ADVERSARY_SIM_LOCAL_RUNTIME_ENV) SPIN_ALWAYS_BUILD=0 ./scripts/run_with_oversight_supervisor.sh spin up --direct-mounts $(SPIN_ENV_ONLY_BASE) $(SPIN_DEV_OVERRIDES) --env SHUMA_KV_STORE_FAIL_OPEN=false --env RUNTIME_INSTANCE_ID=$$RUNTIME_INSTANCE_ID --listen $(LOCAL_CONTRIBUTOR_ORIGIN_LISTEN)'
...
run: ## Build once and run (no file watching)
	...
	@$(LOCAL_LOOPBACK_BAN_CLEANUP_BACKGROUND) RUNTIME_INSTANCE_ID=$$(uuidgen); SHUMA_API_KEY=$(SHUMA_API_KEY) SHUMA_FORWARDED_IP_SECRET=$(SHUMA_FORWARDED_IP_SECRET) SHUMA_ADVERSARY_SIM_SUPERVISOR_BASE_URL=$(LOCAL_CONTRIBUTOR_PUBLIC_BASE_URL) SHUMA_ADVERSARY_SIM_AVAILABLE=$(DEV_ADVERSARY_SIM_AVAILABLE) $(LOCAL_CONTRIBUTOR_INGRESS_ENV) $(ADVERSARY_SIM_LOCAL_RUNTIME_ENV) ./scripts/run_with_oversight_supervisor.sh spin up $(SPIN_ENV_ONLY_BASE) $(SPIN_DEV_OVERRIDES) --env RUNTIME_INSTANCE_ID=$$RUNTIME_INSTANCE_ID --listen $(LOCAL_CONTRIBUTOR_ORIGIN_LISTEN)
...
test: ## Run the canonical local/CI pre-merge suite: code-quality gate, unit, maze verification gate, Spin integration, adversarial matrix, SIM2 gates, and dashboard e2e (excludes live remote proofs)
	@echo "$(CYAN)============================================$(NC)"
	...
	@echo ""
	@echo "$(CYAN)Step 1/9: Code-Quality Gate$(NC)"
	@echo "$(CYAN)--------------------------------------------$(NC)"
	@$(MAKE) --no-print-directory test-code-quality || exit 1
	@echo ""
	@echo "$(CYAN)Step 2/9: Rust Unit Tests$(NC)"
	@echo "$(CYAN)--------------------------------------------$(NC)"
	@./scripts/set_crate_type.sh rlib
	@cargo test || exit 1
	@echo ""
	@echo "$(CYAN)Step 3/9: Maze Verification Gate$(NC)"
	@echo "$(CYAN)--------------------------------------------$(NC)"
	@$(MAKE) --no-print-directory test-maze-verification-gate || exit 1
	@echo ""
	@echo "$(CYAN)Step 4/9: Integration Tests (Spin HTTP scenarios)$(NC)"
	@echo "$(CYAN)--------------------------------------------$(NC)"
	...
	@echo ""
	@echo "$(CYAN)Step 5/9: Runtime Toggle Surface Gate$(NC)"
	@echo "$(CYAN)--------------------------------------------$(NC)"
	@$(MAKE) --no-print-directory test-adversary-sim-runtime-surface || exit 1
	@echo ""
	@echo "$(CYAN)Step 6/9: Adversarial Fast Matrix (smoke + abuse + Akamai)$(NC)"
	@echo "$(CYAN)--------------------------------------------$(NC)"
	@$(MAKE) --no-print-directory test-adversarial-fast || exit 1
	@echo ""
	@echo "$(CYAN)Step 7/9: SIM2 Realtime Verification Gates$(NC)"
	@echo "$(CYAN)--------------------------------------------$(NC)"
	...
	@echo ""
	@echo "$(CYAN)Step 9/9: Loopback Cleanup + Dashboard Seed Snapshot$(NC)"
	@echo "$(CYAN)--------------------------------------------$(NC)"
	@$(MAKE) --no-print-directory clear-dev-loopback-bans || exit 1
...
test-code-quality: ## Run the mandatory non-doc tranche-closing code-quality gate
	@echo "$(CYAN)🧪 Running tranche-closing code-quality gate...$(NC)"
	@$(MAKE) --no-print-directory test-code-quality-contract
	@$(MAKE) --no-print-directory test-native-build-warning-hygiene
	@$(MAKE) --no-print-directory test-dashboard-svelte-check
	@echo "$(GREEN)✅ Mandatory code-quality gate passed.$(NC)"

test-code-quality-contract: ## Validate repo-wide code-quality gate wiring and policy docs
	@echo "$(CYAN)🧪 Running code-quality gate contract checks...$(NC)"
	@python3 -m unittest scripts/tests/test_code_quality_contract.py
	@echo "$(GREEN)✅ Code-quality gate contract holds.$(NC)"

audit-code-quality-deep: ## Run the broader semantic code-quality audit (not yet a tranche-closing gate)
	@echo "$(CYAN)🧪 Running broader semantic code-quality audit...$(NC)"
	@echo "$(YELLOW)This audit currently exposes known repo debt and is intentionally deeper than the mandatory completion gate.$(NC)"
	@$(MAKE) --no-print-directory test-code-quality
	@./scripts/set_crate_type.sh rlib
	...
	@corepack pnpm run test:dashboard:svelte-check:deep
	@echo "$(GREEN)✅ Broader semantic code-quality audit passed.$(NC)"
...
test-runtime-preflight-unit: ## Verify full-suite runtime preflight helper behavior (no server required)
	...
	@python3 -m unittest scripts/tests/test_verify_test_runtime_environment.py scripts/tests/test_set_crate_type.py
...
sim-public-refresh-if-stale: ## Regenerate the contributor sim-public artifact only when missing, source-stale, or older than the bounded freshness window
	@python3 "$(SIM_PUBLIC_SITE_GENERATOR)" \
		--repo-root "$(CURDIR)" \
		--artifact-root "$(SIM_PUBLIC_SITE_ARTIFACT_ROOT)" \
		--corpus-config "$(SIM_PUBLIC_SITE_CORPUS_CONFIG)" \
...
test-sim-public-discoverability-contract: ## Validate generated sim-public discoverability artifacts, pagination, and serving semantics
	...
	@./scripts/set_crate_type.sh rlib
	@cargo test maybe_handle_serves_robots_txt_with_plain_text_content_type -- --nocapture
	@cargo test maybe_handle_serves_sitemap_with_xml_content_type -- --nocapture
	@python3 -m unittest scripts/tests/test_build_sim_public_site.py
...
test-adversarial-lane-realism-contract: ## Validate shared lane realism-profile contract parity across planners and worker tooling
	@echo "$(CYAN)🧪 Validating adversarial lane realism-profile contract...$(NC)"
	@./scripts/set_crate_type.sh rlib
	@cargo test scrapling_worker_plan_surfaces_realism_profile_contract -- --nocapture
	@cargo test llm_fulfillment_plan_surfaces_realism_profile_contract -- --nocapture
	@python3 -m unittest scripts/tests/test_adversarial_lane_realism_contract.py scripts/tests/test_llm_runtime_worker.py
	@if [ ! -x "$(SCRAPLING_VENV_PYTHON)" ]; then \
		echo "$(RED)❌ Error: $(SCRAPLING_VENV_PYTHON) not found.$(NC)"; \
		echo "$(YELLOW)   Run make setup or make setup-runtime to provision the repo-owned Scrapling worker runtime.$(NC)"; \
		exit 1; \
	fi
	@$(SCRAPLING_VENV_PYTHON) -m unittest scripts.tests.test_scrapling_worker.ScraplingWorkerUnitTests.test_execute_worker_plan_rejects_noncanonical_realism_profile
...
test-adversary-sim-lane-selection: ## Focused control-path lane-selection gate (validation, persistence, idempotency, desired-vs-active semantics)
	...
	@cargo test adversary_sim_control_accepts_parallel_mixed_lane_selection_while_off_and_persists_desired_lane -- --nocapture
...
test-adversary-sim-parallel-lane-realism: ## Focused adversary-sim parallel-lane realism gate (bounded mixed Scrapling + Agentic overlap with truthful dispatch proof)
	@echo "$(CYAN)🧪 Running adversary-sim parallel-lane realism gate...$(NC)"
	@./scripts/set_crate_type.sh rlib
	@cargo test autonomous_supervisor_dispatches_parallel_scrapling_and_llm_plans_for_parallel_mixed_lane -- --nocapture
	@cargo test autonomous_supervisor_parallel_mixed_lane_waits_for_both_worker_results_before_redispatching -- --nocapture
	@cargo test adversary_sim_internal_beat_returns_parallel_scrapling_and_llm_dispatch_for_parallel_mixed_lane -- --nocapture
	@python3 -m unittest scripts/tests/test_adversary_sim_supervisor.py
...
test-adversary-sim-shared-observed-telemetry-contract: ## Focused shared-observed sim telemetry gate (request-outcome path must materialize recent-run evidence without worker receipts)
	@echo "$(CYAN)🧪 Running adversary-sim shared observed telemetry contract...$(NC)"
	@./scripts/set_crate_type.sh rlib
	@cargo test runtime::request_outcome::tests::observed_summary_normalizes_request_outcome_for_compact_persistence -- --exact --nocapture
	@cargo test admin::api::tests::recent_sim_run_history_includes_shared_observed_request_outcome_events_without_worker_receipts -- --exact --nocapture

test-adversary-sim-recent-run-observed-coverage: ## Focused recent-run coverage gate (shared observed traffic evidence must drive surface coverage)
	@echo "$(CYAN)🧪 Running adversary-sim recent-run observed coverage gate...$(NC)"
	@./scripts/set_crate_type.sh rlib
	@cargo test admin::api::tests::recent_sim_run_history_projects_shared_observed_surface_coverage_without_receipts -- --exact --nocapture
...
test-adversary-sim-scrapling-realism: ## Focused Scrapling realism gate (profile-driven pacing receipts plus recent-run projection)
	@echo "$(CYAN)🧪 Running adversary-sim Scrapling realism gate...$(NC)"
	@./scripts/set_crate_type.sh rlib
	...
	@$(SCRAPLING_VENV_PYTHON) -m unittest scripts.tests.test_scrapling_worker.ScraplingWorkerUnitTests.test_execute_worker_plan_bulk_scraper_emits_request_realism_receipt
...
test-adversary-sim-recent-run-realism-summaries: ## Focused recent-run realism summary gate (whole-run identity and transport aggregation)
	@echo "$(CYAN)🧪 Running adversary-sim recent-run realism summary gate...$(NC)"
	@./scripts/set_crate_type.sh rlib
	@cargo test admin::api::tests::recent_sim_run_history_projects_latest_scrapling_realism_receipt -- --exact --nocapture
...
test-adversary-sim-pressure-envelope-realism: ## Focused adversary-sim pressure-envelope gate (mode-specific worker ceilings plus concurrent request-mode bursts)
	...
	@$(SCRAPLING_VENV_PYTHON) -m unittest scripts.tests.test_scrapling_worker.ScraplingWorkerUnitTests.test_realism_tracker_respects_bulk_scraper_pressure_envelope_above_legacy_flat_cap
	@python3 -m unittest scripts/tests/test_llm_runtime_worker.py
	@python3 -m unittest scripts/tests/test_adversarial_container_worker.py
...
test-adversarial-identity-envelope-contract: ## Focused adversary-sim identity-envelope gate (contract shape, degraded honesty, and plan emission)
	...
	@./scripts/set_crate_type.sh rlib
	@cargo test admin::api::admin_config_tests::adversary_sim_internal_beat_returns_scrapling_worker_plan_and_switches_active_lane -- --exact --nocapture
	@cargo test admin::api::admin_config_tests::adversary_sim_internal_beat_returns_llm_fulfillment_plan_for_bot_red_team_lane -- --exact --nocapture
	...
	@$(SCRAPLING_VENV_PYTHON) -m unittest scripts.tests.test_scrapling_worker.ScraplingWorkerUnitTests.test_realism_tracker_marks_degraded_identity_realism_without_pool

test-adversary-sim-header-transport-realism: ## Focused adversary-sim header/locale/transport realism gate (coherent persona envelopes plus truthful receipts)
	@echo "$(CYAN)🧪 Running adversary-sim header/locale/transport realism gate...$(NC)"
	@python3 -m unittest scripts/tests/test_adversarial_lane_realism_contract.py scripts/tests/test_llm_runtime_worker.py scripts/tests/test_adversarial_container_worker.py
	@python3 -m unittest scripts/tests/test_adversary_sim_make_targets.py
	@if [ ! -x "$(SCRAPLING_VENV_PYTHON)" ]; then \
		echo "$(RED)❌ Error: $(SCRAPLING_VENV_PYTHON) not found.$(NC)"; \
...
test-client-ip-topology-contract: ## Focused client-IP topology gate (shared-host direct/trusted/misconfigured, edge fallback, and /shuma/health hop rules)
	...
	@./scripts/set_crate_type.sh rlib
	@cargo test lib_tests::security::shared_host_direct_requests_collapse_client_ip_to_unknown -- --exact --nocapture
	@cargo test lib_tests::security::shared_host_trusted_proxy_uses_forwarded_for_for_client_ip_extraction -- --exact --nocapture
	@cargo test lib_tests::security::shared_host_misconfigured_proxy_headers_fail_closed_to_unknown -- --exact --nocapture
	@cargo test lib_tests::security::edge_fermyon_uses_true_client_ip_for_client_ip_extraction -- --exact --nocapture
	@cargo test lib_tests::security::edge_fermyon_falls_back_to_trusted_forwarded_for_when_true_client_ip_missing -- --exact --nocapture
	@cargo test lib_tests::security::edge_fermyon_without_trusted_identity_headers_returns_unknown -- --exact --nocapture
	@cargo test lib_tests::security::health_ip_extraction_accepts_single_hop_trusted_forwarded_for -- --exact --nocapture
	@cargo test lib_tests::security::health_ip_extraction_rejects_multi_hop_forwarded_for -- --exact --nocapture

test-adversary-sim-trusted-ingress-ip-realism: ## Focused trusted-ingress realism gate (worker proxy path restores parseable IPs without privileged worker headers)
	@echo "$(CYAN)🧪 Running adversary-sim trusted-ingress IP realism gate...$(NC)"
	@$(MAKE) --no-print-directory test-client-ip-topology-contract
	@$(MAKE) --no-print-directory test-adversarial-lane-contract
	@./scripts/set_crate_type.sh rlib
...
test-adversary-sim-identity-observer-truth: ## Focused identity-observer truth gate (compact provenance receipts plus truthful Red Team and Game Loop wording)
	@echo "$(CYAN)🧪 Running adversary-sim identity-observer truth gate...$(NC)"
	@./scripts/set_crate_type.sh rlib
	@cargo test admin::api::tests::recent_sim_run_history_projects_llm_runtime_receipts_and_categories -- --exact --nocapture
	@cargo test admin::api::tests::recent_sim_run_history_projects_latest_scrapling_realism_receipt -- --exact --nocapture
	@cargo test observability::operator_snapshot::tests::snapshot_payload_projects_llm_runtime_summary -- --exact --nocapture
	@cargo test observability::operator_snapshot::tests::snapshot_payload_projects_latest_scrapling_recent_run_identity_receipt -- --exact --nocapture
	@python3 -m unittest scripts/tests/test_llm_runtime_worker.py
	@if [ ! -x "$(SCRAPLING_VENV_PYTHON)" ]; then \
		echo "$(RED)❌ Error: $(SCRAPLING_VENV_PYTHON) not found.$(NC)"; \
		echo "$(YELLOW)   Run make setup or make setup-runtime to provision the repo-owned Scrapling worker runtime.$(NC)"; \
	...
	@$(SCRAPLING_VENV_PYTHON) -m unittest scripts.tests.test_scrapling_worker.ScraplingWorkerUnitTests
	@if ! command -v corepack >/dev/null 2>&1; then \
		echo "$(RED)❌ Error: corepack not found (install Node.js 18+).$(NC)"; \
		exit 1; \
	fi
	...
	@if $(MAKE) --no-print-directory spin-wait-ready; then \
		...
		$(MAKE) --no-print-directory seed-dashboard-data || exit 1; \
		SHUMA_BASE_URL=http://127.0.0.1:3000 SHUMA_API_KEY=$(SHUMA_API_KEY) SHUMA_FORWARDED_IP_SECRET=$(SHUMA_FORWARDED_IP_SECRET) ./scripts/tests/run_dashboard_e2e.sh --grep "red team recent runs label trusted and degraded identity realism truthfully|game loop observer cast labels recent-run identity realism truthfully"; \
	else \
		echo "$(RED)❌ Error: Spin server not ready$(NC)"; \
		echo "$(YELLOW)   Start the server first: make dev$(NC)"; \
		exit 1; \
...
test-adversarial-llm-realism: ## Focused bounded LLM realism gate (request-mode burst or pause shaping, worker receipt truth, and recent-run projection)
	@echo "$(CYAN)🧪 Running bounded adversarial LLM realism gate...$(NC)"
	@python3 -m unittest scripts/tests/test_llm_runtime_worker.py
	@python3 -m unittest scripts/tests/test_adversarial_container_worker.py
	@./scripts/set_crate_type.sh rlib
...
test-adversarial-llm-browser-runtime: ## Focused bounded LLM browser-mode runtime gate (real browser session execution, receipt truth, and recent-run projection)
	@echo "$(CYAN)🧪 Running bounded adversarial LLM browser-mode runtime gate...$(NC)"
	@python3 -m unittest scripts/tests/test_llm_runtime_worker.py
	@corepack pnpm exec node scripts/tests/test_adversarial_browser_driver.mjs
	@if $(MAKE) --no-print-directory spin-wait-ready; then \
		SHUMA_BASE_URL=http://127.0.0.1:3000 python3 -m unittest scripts/tests/test_llm_runtime_browser_integration.py; \
	else \
		echo "$(RED)❌ Error: Spin server not ready$(NC)"; \
		echo "$(YELLOW)   Start the server first: make dev$(NC)"; \
		exit 1; \
...
test-adversary-sim-agentic-action-realism: ## Focused agentic action-surface and degraded-fallback realism gate
	@echo "$(CYAN)🧪 Running agentic action-surface realism gate...$(NC)"
	@$(MAKE) --no-print-directory test-adversarial-lane-realism-contract
	@$(MAKE) --no-print-directory test-adversarial-llm-fit
	@$(MAKE) --no-print-directory test-adversarial-llm-realism
	@$(MAKE) --no-print-directory test-adversarial-llm-browser-runtime
...
test-dashboard-adversary-sim-lane-contract: ## Run focused dashboard lane-contract checks for adversary-sim controls
	...
	@$(MAKE) --no-print-directory test-dashboard-svelte-check
	@node --test \
		--test-name-pattern='dashboard API client preserves adversary-sim lane status and diagnostics fields|dashboard API client sends optional adversary-sim lane selection in control writes|dashboard adversary-sim runtime normalizes orchestration status|dashboard adversary-sim lane labels collapse legacy deterministic alias to synthetic traffic|dashboard adversary-sim lifecycle copy prioritizes controller convergence over steady-state backend copy|dashboard red team controller can replace backend status after lane-only control writes|red team auto-refresh rehydrates missing config runtime write truth for lane controls|red team tab reuses verification-style config panel primitives for its adversary sim pane|adversary run panel separates execution identity and transport columns|dashboard route lazily loads heavy tabs and keeps orchestration local' \
		e2e/dashboard.modules.unit.test.js

test-dashboard-red-team-lane-selector-contract: ## Focused dashboard Red Team lane-selector contract gate (agentic plus parallel mixed lane control truth)
	@echo "$(CYAN)🧪 Running focused dashboard Red Team lane-selector contract checks...$(NC)"
	@if ! command -v corepack >/dev/null 2>&1; then \
		echo "$(RED)❌ Error: corepack not found (install Node.js 18+).$(NC)"; \
		exit 1; \
	fi
	...
	@$(MAKE) --no-print-directory test-dashboard-svelte-check
	@node --test \
		--test-name-pattern='red team tab reuses verification-style config panel primitives for its adversary sim pane' \
		e2e/dashboard.modules.unit.test.js
	@$(MAKE) --no-print-directory runtime-verify-freshness || exit 1;
	@$(MAKE) --no-print-directory dashboard-verify-freshness || exit 1;
	@./scripts/tests/verify_served_dashboard_assets.sh http://127.0.0.1:3000 || exit 1;
	@$(MAKE) --no-print-directory seed-dashboard-data
	@SHUMA_BASE_URL=http://127.0.0.1:3000 SHUMA_API_KEY=$(SHUMA_API_KEY) SHUMA_FORWARDED_IP_SECRET=$(SHUMA_FORWARDED_IP_SECRET) ./scripts/tests/run_dashboard_e2e.sh --grep "adversary sim lane selector keeps off-state desired truth and allows agentic and parallel mixed traffic"
...
test-dashboard-hash-tab-boot-contract: ## Run focused dashboard direct-hash boot checks for admin tabs
	@echo "$(CYAN)🧪 Running focused dashboard hash-tab boot checks...$(NC)"
	@if ! command -v corepack >/dev/null 2>&1; then \
		echo "$(RED)❌ Error: corepack not found (install Node.js 18+).$(NC)"; \
		exit 1; \
	fi
	...
	@$(MAKE) --no-print-directory test-dashboard-svelte-check
	@node --test \
		--test-name-pattern='dashboard route preserves an empty server-side hash bootstrap so client hash can select the first tab load' \
		e2e/dashboard.modules.unit.test.js
	@if $(MAKE) --no-print-directory spin-wait-ready; then \
		$(MAKE) --no-print-directory seed-dashboard-data || exit 1; \
		SHUMA_BASE_URL=http://127.0.0.1:3000 SHUMA_API_KEY=$(SHUMA_API_KEY) SHUMA_FORWARDED_IP_SECRET=$(SHUMA_FORWARDED_IP_SECRET) ./scripts/tests/run_dashboard_e2e.sh --grep "direct hash loads admin tabs on first render"; \
	else \
		echo "$(RED)❌ Error: Spin server not ready$(NC)"; \
		echo "$(YELLOW)   Start the server first: make dev$(NC)"; \
		exit 1; \
...
test-dashboard-game-loop-accountability: ## Run focused dashboard Game Loop observer-accountability checks
	...
	@if $(MAKE) --no-print-directory spin-wait-ready; then \
		...
		./scripts/tests/verify_served_dashboard_assets.sh http://127.0.0.1:3000 || exit 1; \
		$(MAKE) --no-print-directory seed-dashboard-data || exit 1; \
		SHUMA_BASE_URL=http://127.0.0.1:3000 SHUMA_API_KEY=$(SHUMA_API_KEY) SHUMA_FORWARDED_IP_SECRET=$(SHUMA_FORWARDED_IP_SECRET) ./scripts/tests/run_dashboard_e2e.sh --grep "game loop projects observer-facing round, adversary, and defence accountability from machine-first contracts|game loop observer archive marks missing judged runs without guessing adversary or defence casts|game loop top casts prefer the freshest exact recent sim run over stale judged history"; \
	else \
		echo "$(RED)❌ Error: Spin server not ready$(NC)"; \
		echo "$(YELLOW)   Start the server first: make dev$(NC)"; \
		exit 1; \
...
test-dashboard-diagnostics-pane: ## Run focused dashboard Diagnostics pane ownership checks
	...
	@if $(MAKE) --no-print-directory spin-wait-ready; then \
		$(MAKE) --no-print-directory seed-dashboard-data || exit 1; \
		SHUMA_BASE_URL=http://127.0.0.1:3000 SHUMA_API_KEY=$(SHUMA_API_KEY) SHUMA_FORWARDED_IP_SECRET=$(SHUMA_FORWARDED_IP_SECRET) ./scripts/tests/run_dashboard_e2e.sh --grep "diagnostics tab keeps subsystem inspection surfaces after overview-rollup retirement|game loop, traffic, and diagnostics tabs expose their ownership split"; \
	else \
		echo "$(RED)❌ Error: Spin server not ready$(NC)"; \
		echo "$(YELLOW)   Start the server first: make dev$(NC)"; \
		exit 1; \
...
test-dashboard-disconnect-behavior-contract: ## Run focused dashboard disconnect/backoff behavior checks
	@echo "$(CYAN)🧪 Running focused dashboard disconnect behavior checks...$(NC)"
	@if ! command -v corepack >/dev/null 2>&1; then \
		echo "$(RED)❌ Error: corepack not found (install Node.js 18+).$(NC)"; \
		exit 1; \
	fi
	...
	@$(MAKE) --no-print-directory test-dashboard-svelte-check
	@node --test \
		--test-name-pattern='dashboard red team controller returns settled promises for passive refresh triggers when status fetch fails|dashboard route controller suppresses auto polling while backend connection is disconnected|dashboard native runtime derives bounded heartbeat retry backoff from consecutive failures|dashboard route wires backend disconnect polling guards and reconnect resume through the heartbeat-owned connection state' \
		e2e/dashboard.modules.unit.test.js
...
test-dashboard-config-refresh-contract: ## Run focused dashboard config refresh and restore-cleanup checks
	@echo "$(CYAN)🧪 Running focused dashboard config refresh and restore-cleanup checks...$(NC)"
	@./scripts/set_crate_type.sh rlib
	@cargo test admin::api::admin_config_tests::admin_config_responses_disable_caching -- --exact --nocapture
	@if ! command -v corepack >/dev/null 2>&1; then \
		echo "$(RED)❌ Error: corepack not found (install Node.js 18+).$(NC)"; \
		exit 1; \
	fi
	...
	@$(MAKE) --no-print-directory test-dashboard-svelte-check
	@node --test \
		--test-name-pattern='dashboard smoke policy restore paths cover every config family the smoke mutates|dashboard config-backed tab refreshes shared config from the server on click even when a snapshot exists' \
		e2e/dashboard.modules.unit.test.js
	@if $(MAKE) --no-print-directory spin-wait-ready; then \
		$(MAKE) --no-print-directory dashboard-verify-freshness || exit 1; \
		./scripts/tests/verify_served_dashboard_assets.sh http://127.0.0.1:3000 || exit 1; \
		SHUMA_BASE_URL=http://127.0.0.1:3000 SHUMA_API_KEY=$(SHUMA_API_KEY) SHUMA_FORWARDED_IP_SECRET=$(SHUMA_FORWARDED_IP_SECRET) ./scripts/tests/run_dashboard_e2e.sh --grep "config-backed tabs rehydrate external config mutations without surfacing unsaved changes"; \
	else \
		echo "$(RED)❌ Error: Spin server not ready$(NC)"; \
		echo "$(YELLOW)   Start the server first: make dev$(NC)"; \
		exit 1; \
...
test-dashboard-verified-identity-pane: ## Run focused Verification-tab verified-identity control checks
	@echo "$(CYAN)🧪 Running focused dashboard verified-identity control checks...$(NC)"
	@if ! command -v corepack >/dev/null 2>&1; then \
		echo "$(RED)❌ Error: corepack not found (install Node.js 18+).$(NC)"; \
		exit 1; \
	fi
	...
	@if $(MAKE) --no-print-directory spin-wait-ready; then \
		$(MAKE) --no-print-directory seed-dashboard-data || exit 1; \
		SHUMA_BASE_URL=http://127.0.0.1:3000 SHUMA_API_KEY=$(SHUMA_API_KEY) SHUMA_FORWARDED_IP_SECRET=$(SHUMA_FORWARDED_IP_SECRET) ./scripts/tests/run_dashboard_e2e.sh --grep "verification tab surfaces verified identity controls"; \
	else \
		echo "$(RED)❌ Error: Spin server not ready$(NC)"; \
		echo "$(YELLOW)   Start the server first: make dev$(NC)"; \
		exit 1; \
...
test-dashboard-status-pane: ## Run focused Status-tab operator-health and verified-identity summary checks
	@echo "$(CYAN)🧪 Running focused dashboard Status tab checks...$(NC)"
	@if ! command -v corepack >/dev/null 2>&1; then \
		echo "$(RED)❌ Error: corepack not found (install Node.js 18+).$(NC)"; \
		exit 1; \
	fi
	...
	@$(MAKE) --no-print-directory test-dashboard-svelte-check
	@node --test \
		--test-name-pattern='dashboard status tab wires verified identity operator snapshot and store state|dashboard config tabs reuse shared panels, save flows, and owned controls' \
		e2e/dashboard.modules.unit.test.js
	@if $(MAKE) --no-print-directory spin-wait-ready; then \
		$(MAKE) --no-print-directory seed-dashboard-data || exit 1; \
		SHUMA_BASE_URL=http://127.0.0.1:3000 SHUMA_API_KEY=$(SHUMA_API_KEY) SHUMA_FORWARDED_IP_SECRET=$(SHUMA_FORWARDED_IP_SECRET) ./scripts/tests/run_dashboard_e2e.sh --grep "status tab surfaces verified identity health summary"; \
	else \
		echo "$(RED)❌ Error: Spin server not ready$(NC)"; \
		echo "$(YELLOW)   Start the server first: make dev$(NC)"; \
		exit 1; \
...
test-dashboard-fingerprint-control-rehome: ## Run focused dashboard verification/tuning fingerprint-control rehome checks
	@echo "$(CYAN)🧪 Running focused dashboard verification/tuning fingerprint-control rehome checks...$(NC)"
	@if ! command -v corepack >/dev/null 2>&1; then \
		echo "$(RED)❌ Error: corepack not found (install Node.js 18+).$(NC)"; \
		exit 1; \
	fi
	...
	@$(MAKE) --no-print-directory test-dashboard-svelte-check
	@node --test \
		--test-name-pattern='dashboard config tabs reuse shared panels, save flows, and owned controls|dashboard verification tab wires verified identity operator snapshot and store state' \
		e2e/dashboard.modules.unit.test.js
	@if $(MAKE) --no-print-directory spin-wait-ready; then \
		$(MAKE) --no-print-directory dashboard-verify-freshness || exit 1; \
		./scripts/tests/verify_served_dashboard_assets.sh http://127.0.0.1:3000 || exit 1; \
		$(MAKE) --no-print-directory seed-dashboard-data || exit 1; \
		SHUMA_BASE_URL=http://127.0.0.1:3000 SHUMA_API_KEY=$(SHUMA_API_KEY) SHUMA_FORWARDED_IP_SECRET=$(SHUMA_FORWARDED_IP_SECRET) ./scripts/tests/run_dashboard_e2e.sh --grep "verification tab surfaces verified identity controls and health summary|geo and tuning save flows cover GEO lists and botness controls|verification tab hides Akamai controls outside edge-fermyon posture"; \
	else \
		echo "$(RED)❌ Error: Spin server not ready$(NC)"; \
		echo "$(YELLOW)   Start the server first: make dev$(NC)"; \
		exit 1; \
...
test-dashboard-red-team-pane: ## Run focused Red Team controls and recent-run surfacing checks
	@echo "$(CYAN)🧪 Running focused dashboard Red Team pane checks...$(NC)"
	@if ! command -v corepack >/dev/null 2>&1; then \
		echo "$(RED)❌ Error: corepack not found (install Node.js 18+).$(NC)"; \
		exit 1; \
	fi
	...
	@$(MAKE) --no-print-directory test-dashboard-svelte-check
	@node --test \
		--test-name-pattern='dashboard API client preserves adversary-sim lane status and diagnostics fields|dashboard adversary-sim runtime normalizes orchestration status|monitoring view model formats transport realism summaries for recent adversary runs|recent adversary run summaries preserve whole-run scrapling activity totals over latest receipt counts|recent adversary run summaries prefer shared observed coverage over receipt-backed fallback|recent adversary run summaries mark receipt-only agentic rows as unobserved shared telemetry and count response coverage honestly|red team tab reuses verification-style config panel primitives for its adversary sim pane|red team tab keeps only controls and recent adversary runs after diagnostic-surface retirement|adversary run panel separates execution identity and transport columns' \
		e2e/dashboard.modules.unit.test.js
	@if $(MAKE) --no-print-directory spin-wait-ready; then \
		$(MAKE) --no-print-directory dashboard-verify-freshness || exit 1; \
		./scripts/tests/verify_served_dashboard_assets.sh http://127.0.0.1:3000 || exit 1; \
		$(MAKE) --no-print-directory seed-dashboard-data || exit 1; \
		SHUMA_BASE_URL=http://127.0.0.1:3000 SHUMA_API_KEY=$(SHUMA_API_KEY) SHUMA_FORWARDED_IP_SECRET=$(SHUMA_FORWARDED_IP_SECRET) ./scripts/tests/run_dashboard_e2e.sh --grep "red team tab keeps controls and recent run history without machine-diagnostic clutter|adversary sim lane selector keeps off-state desired truth and allows agentic traffic|red team recent runs label identity and transport realism truthfully"; \
	else \
		echo "$(RED)❌ Error: Spin server not ready$(NC)"; \
		echo "$(YELLOW)   Start the server first: make dev$(NC)"; \
		exit 1; \
...
test-dashboard-adversary-sim-telemetry-contract: ## Run focused dashboard adversary-sim telemetry smoke without Scrapling-history pollution
	@echo "$(CYAN)🧪 Running focused dashboard adversary-sim telemetry contract...$(NC)"
	@if ! command -v corepack >/dev/null 2>&1; then \
		echo "$(RED)❌ Error: corepack is required for dashboard unit tests$(NC)"; \
		exit 1; \
	fi
	@corepack enable > /dev/null 2>&1 || true
	@if [ ! -d node_modules/.pnpm ] || [ ! -x node_modules/.bin/vite ] || [ ! -x node_modules/.bin/svelte-check ] || [ ! -d node_modules/svelte ] || [ ! -d node_modules/@sveltejs/kit ] || [ ! -d node_modules/@playwright/test ]; then \
	...
	@$(MAKE) --no-print-directory test-dashboard-svelte-check
	@node --test \
		--test-name-pattern='dashboard adversary-sim telemetry smoke uses synthetic traffic instead of leaving truncated Scrapling runs in recent history|dashboard adversary-sim lifecycle smokes use synthetic traffic unless they are explicitly proving lane-specific recent-run materialization|dashboard adversary-sim agentic telemetry smoke explicitly drives bot_red_team recent-run proof|dashboard hosted live smoke pins synthetic traffic before UI-only adversary toggle proof' \
		e2e/dashboard.modules.unit.test.js
	@if $(MAKE) --no-print-directory spin-wait-ready; then \
		$(MAKE) --no-print-directory runtime-verify-freshness || exit 1; \
		$(MAKE) --no-print-directory dashboard-verify-freshness || exit 1; \
		$(MAKE) --no-print-directory seed-dashboard-data || exit 1; \
		SHUMA_BASE_URL=http://127.0.0.1:3000 SHUMA_API_KEY=$(SHUMA_API_KEY) SHUMA_FORWARDED_IP_SECRET=$(SHUMA_FORWARDED_IP_SECRET) ./scripts/tests/run_dashboard_e2e.sh --grep "adversary sim toggle emits fresh telemetry visible in monitoring raw feed|adversary sim agentic toggle materializes shared recent-run telemetry in red team table"; \
	else \
		echo "$(RED)❌ Error: Spin server not ready$(NC)"; \
		echo "$(YELLOW)   Start the server first: make dev$(NC)"; \
		exit 1; \
...
test-adversarial-llm-runtime-frontier-env-parity: ## Focused local frontier env parity gate (host-side Agentic worker must inherit configured frontier env and materialize shared recent-run telemetry)
	@echo "$(CYAN)🧪 Running adversarial LLM runtime frontier-env parity gate...$(NC)"
	@$(MAKE) --no-print-directory test-runtime-preflight-unit
	@$(MAKE) --no-print-directory test-adversary-sim-make-target-contract
	@$(MAKE) --no-print-directory test-adversarial-llm-runtime-dispatch
	@$(MAKE) --no-print-directory test-dashboard-adversary-sim-telemetry-contract

test-adversary-sim-transport-fingerprint-realism: ## Focused adversary-sim transport realism gate (achieved transport class, degraded truth, and observer surfacing)
	@echo "$(CYAN)🧪 Running adversary-sim transport-fingerprint realism gate...$(NC)"
	@$(MAKE) --no-print-directory test-adversary-sim-header-transport-realism
	@./scripts/set_crate_type.sh rlib
	@cargo test admin::api::tests::recent_sim_run_history_projects_llm_runtime_receipts_and_categories -- --exact --nocapture
	@cargo test admin::api::tests::recent_sim_run_history_projects_llm_browser_secondary_traffic_receipt_counts -- --exact --nocapture
	@cargo test admin::api::tests::recent_sim_run_history_projects_latest_scrapling_realism_receipt -- --exact --nocapture
	@cargo test observability::operator_snapshot::tests::snapshot_payload_projects_recent_run_llm_runtime_summary -- --exact --nocapture
	@cargo test observability::operator_snapshot::tests::snapshot_payload_projects_llm_browser_secondary_traffic_receipt_counts -- --exact --nocapture
	@cargo test observability::operator_snapshot::tests::snapshot_payload_projects_latest_scrapling_recent_run_identity_receipt -- --exact --nocapture
	@node --test \
		--test-name-pattern='monitoring view model formats transport realism summaries for recent adversary runs' \
		e2e/dashboard.modules.unit.test.js
	@$(MAKE) --no-print-directory test-dashboard-red-team-pane
	@python3 -m unittest scripts/tests/test_adversary_sim_make_targets.py

test-adversary-sim-representativeness-readiness: ## Focused adversary-sim representativeness readiness gate (infrastructure truth and Red Team operator surfacing)
	@echo "$(CYAN)🧪 Running adversary-sim representativeness readiness gate...$(NC)"
	@./scripts/set_crate_type.sh rlib
	@cargo test admin::adversary_sim_representativeness::tests::representativeness_readiness_is_degraded_without_realism_infrastructure -- --exact --nocapture
	@cargo test admin::adversary_sim_representativeness::tests::representativeness_readiness_is_partial_with_trusted_ingress_only -- --exact --nocapture
	@cargo test admin::adversary_sim_representativeness::tests::representativeness_readiness_is_representative_when_hostile_prerequisites_exist -- --exact --nocapture
	@cargo test admin::api::admin_config_tests::adversary_sim_status_surfaces_representativeness_readiness_truth -- --exact --nocapture
	@$(MAKE) --no-print-directory test-dashboard-red-team-pane
	@python3 -m unittest scripts/tests/test_adversary_sim_make_targets.py
...
test-dashboard-e2e-adversary-sim: ## Run focused Playwright adversary-sim dashboard smoke checks
	...
	@if $(MAKE) --no-print-directory spin-wait-ready; then \
		...
		$(MAKE) --no-print-directory seed-dashboard-data || exit 1; \
		SHUMA_BASE_URL=http://127.0.0.1:3000 SHUMA_API_KEY=$(SHUMA_API_KEY) SHUMA_FORWARDED_IP_SECRET=$(SHUMA_FORWARDED_IP_SECRET) ./scripts/tests/run_dashboard_e2e.sh --grep "adversary sim (global toggle drives orchestration control lifecycle state|toggle emits fresh telemetry visible in monitoring raw feed|lane selector keeps off-state desired truth and allows agentic and parallel mixed traffic)"; \
	else \
		echo "$(RED)❌ Error: Spin server not ready$(NC)"; \
		echo "$(YELLOW)   Start the server first: make dev$(NC)"; \
		exit 1; \
...
clear-dev-loopback-bans: ## Clear local dev loopback-style bans (127.0.0.1, ::1, unknown) from the running server
	@echo "$(CYAN)🧪 Clearing local dev loopback bans...$(NC)"
	@if $(MAKE) --no-print-directory spin-wait-ready; then \
		if [[ -z "$(SHUMA_API_KEY)" ]]; then \
			echo "$(RED)❌ Error: SHUMA_API_KEY is required to clear loopback bans.$(NC)"; \
			echo "$(YELLOW)   Run make setup or export SHUMA_API_KEY before invoking this target.$(NC)"; \
			exit 1; \
		fi; \
		for ip in 127.0.0.1 ::1 unknown; do \
			status="$$(curl -s -o /tmp/shuma_clear_loopback_ban_body.txt -w "%{http_code}" \
				-H "Authorization: Bearer $(SHUMA_API_KEY)" \
				-H "X-Forwarded-For: 127.0.0.1" \
				$(FORWARDED_SECRET_HEADER) \
				-X POST "http://127.0.0.1:3000/shuma/admin/unban?ip=$$ip")"; \
			if [[ "$$status" != "200" ]]; then \
				echo "$(RED)❌ Error: failed to clear loopback ban for $$ip (status=$$status).$(NC)"; \
				cat /tmp/shuma_clear_loopback_ban_body.txt; \
				rm -f /tmp/shuma_clear_loopback_ban_body.txt; \
				exit 1; \
			fi; \
		done; \
		rm -f /tmp/shuma_clear_loopback_ban_body.txt; \
...
test-local-contributor-ingress-contract: ## Validate local contributor ingress contract for direct browsing and dev wiring
	@echo "$(CYAN)🧪 Running local contributor ingress contract checks...$(NC)"
	@python3 -m unittest scripts/tests/test_trusted_ingress_proxy.py scripts/tests/test_integration_cleanup.py scripts/tests/test_adversary_sim_supervisor.py

test-local-contributor-root-access-contract: ## Prove the local public root stays accessible when unknown is banned at the internal origin
	@echo "$(CYAN)🧪 Running local contributor root-access cleanup proof...$(NC)"
	@if $(MAKE) --no-print-directory spin-wait-ready; then \
		if [[ -z "$(SHUMA_API_KEY)" ]]; then \
			echo "$(RED)❌ Error: SHUMA_API_KEY is required for local contributor root-access proof.$(NC)"; \
			exit 1; \
		fi; \
		if [[ -z "$(SHUMA_FORWARDED_IP_SECRET)" ]]; then \
			echo "$(RED)❌ Error: SHUMA_FORWARDED_IP_SECRET is required for local contributor root-access proof.$(NC)"; \
			exit 1; \
		fi; \
		$(MAKE) --no-print-directory test-local-contributor-ingress-contract || exit 1; \
		SHUMA_BASE_URL=$(LOCAL_CONTRIBUTOR_PUBLIC_BASE_URL) SHUMA_LOCAL_CONTRIBUTOR_ORIGIN_BASE_URL=$(LOCAL_CONTRIBUTOR_ORIGIN_BASE_URL) SHUMA_API_KEY="$(SHUMA_API_KEY)" SHUMA_FORWARDED_IP_SECRET="$(SHUMA_FORWARDED_IP_SECRET)" \
			python3 scripts/tests/verify_local_root_access_cleanup.py; \
	else \
		echo "$(RED)❌ Error: Spin server not ready$(NC)"; \
		echo "$(YELLOW)   Start the server first: make dev$(NC)"; \
		exit 1; \
...
test-local-contributor-sim-isolation-contract: ## Prove local adversary sim traffic does not ban the contributor-facing root identity
	@echo "$(CYAN)🧪 Running local contributor sim-isolation proof...$(NC)"
	@if $(MAKE) --no-print-directory spin-wait-ready; then \
		if [[ -z "$(SHUMA_API_KEY)" ]]; then \
			echo "$(RED)❌ Error: SHUMA_API_KEY is required for local contributor sim-isolation proof.$(NC)"; \
			exit 1; \
		fi; \
		if [[ -z "$(SHUMA_FORWARDED_IP_SECRET)" ]]; then \
			echo "$(RED)❌ Error: SHUMA_FORWARDED_IP_SECRET is required for local contributor sim-isolation proof.$(NC)"; \
			exit 1; \
		fi; \
		$(MAKE) --no-print-directory test-local-contributor-ingress-contract || exit 1; \
...
test-local-contributor-self-contained-public-site-contract: ## Prove local contributor public-site misses stay local and do not fall through to upstream gateway forwarding
	@echo "$(CYAN)🧪 Running local contributor self-contained public-site proof...$(NC)"
	@if $(MAKE) --no-print-directory spin-wait-ready; then \
		$(MAKE) --no-print-directory test-local-contributor-ingress-contract || exit 1; \
		./scripts/set_crate_type.sh rlib || exit 1; \
		cargo test config::tests::local_contributor_ingress_enabled_defaults_false_and_honors_env_override -- --exact --nocapture || exit 1; \
		cargo test runtime::sim_public::tests::sim_public_relative_asset_path_claims_unknown_public_paths_for_local_contributor_mode -- --exact --nocapture || exit 1; \
		cargo test runtime::sim_public::tests::maybe_handle_returns_local_not_found_for_unknown_public_paths_in_local_contributor_mode -- --exact --nocapture || exit 1; \
		python3 -m unittest scripts/tests/test_config_lifecycle.py || exit 1; \
		SHUMA_BASE_URL=$(LOCAL_CONTRIBUTOR_PUBLIC_BASE_URL) python3 scripts/tests/verify_local_public_site_self_contained.py; \
	else \
		echo "$(RED)❌ Error: Spin server not ready$(NC)"; \
		echo "$(YELLOW)   Start the server first: make dev$(NC)"; \
		exit 1; \
...
env-help: ## Show supported env-only runtime overrides
	...
	@echo "  SHUMA_LOCAL_CONTRIBUTOR_INGRESS_ENABLE"
