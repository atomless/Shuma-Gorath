...
test-oversight-move-selection-policy: ## Run focused RSI-GAME-1B move-selection policy checks
	...
	@cargo test observability::benchmark_results_comparison::tests::escalation_hint_marks_category_posture_gap_as_code_evolution_only -- --exact --nocapture
...
test-rsi-game-human-only-cycles: ## Run focused repeated strict human-only loop cycle proof checks
	@echo "$(CYAN)🧪 Running repeated strict human-only loop cycle proof checks...$(NC)"
	@./scripts/set_crate_type.sh rlib
	@cargo test post_sim_oversight_route_repeats_human_only_cycles_with_retained_then_rolled_back_config -- --nocapture

test-rsi-game-human-only-improvement: ## Run focused strict human-only unlock-condition improvement proof checks
	@echo "$(CYAN)🧪 Running strict human-only unlock-condition proof checks...$(NC)"
	@./scripts/set_crate_type.sh rlib
...
test-scrapling-game-loop-mainline: ## Run the current active mainline proof bundle (attacker-faithful Scrapling plus strict human-only unlock proofs)
	...
	@$(MAKE) --no-print-directory test-rsi-game-human-only-cycles
	@$(MAKE) --no-print-directory test-rsi-game-human-only-improvement
...
test-adversarial-llm-runtime-proof: ## Focused bounded LLM runtime proof gate (dispatch plus recent-run/operator-snapshot projection)
	@echo "$(CYAN)🧪 Running bounded adversarial LLM runtime proof gate...$(NC)"
	@$(MAKE) --no-print-directory test-adversarial-llm-runtime-dispatch
	@./scripts/set_crate_type.sh rlib
	@cargo test observability::llm_runtime_recent_run::tests:: -- --nocapture
	@cargo test observability::non_human_lane_fulfillment::tests::runtime_profile_observation_helper_normalizes_bot_red_team_modes_into_categories -- --exact --nocapture
	@cargo test admin::api::tests::recent_sim_run_history_projects_llm_runtime_receipts_for_bot_red_team -- --exact --nocapture

test-adversarial-llm-browser-runtime: ## Focused bounded LLM browser-mode runtime gate (container browser execution plus recent-run/dashboard proof)
	@echo "$(CYAN)🧪 Running bounded adversarial LLM browser-mode runtime gate...$(NC)"
	@$(MAKE) --no-print-directory test-adversarial-llm-fit
	@python3 -m py_compile scripts/tests/adversarial_container_runner.py scripts/tests/adversarial_container/worker.py
	@python3 -m unittest scripts/tests/test_adversarial_container_runner.py scripts/tests/test_adversarial_container_worker.py
	@$(MAKE) --no-print-directory test-adversarial-llm-runtime-proof
	@$(MAKE) --no-print-directory test-dashboard-red-team-llm-runtime
...
test-dashboard-red-team-llm-runtime: ## Run focused Red Team bot_red_team browser-runtime rendering checks
	@echo "$(CYAN)🧪 Running focused dashboard Red Team bot_red_team browser-runtime checks...$(NC)"
	@if ! command -v corepack >/dev/null 2>&1; then \
		echo "$(RED)❌ Error: corepack not found (install Node.js 18+).$(NC)"; \
		exit 1; \
	fi
	...
	@$(MAKE) --no-print-directory test-dashboard-svelte-check
	@node --test \
		--test-name-pattern='monitoring view model and status module remain pure snapshot transforms|red team tab reuses verification-style config panel primitives for its adversary sim pane' \
		e2e/dashboard.modules.unit.test.js
	@if $(MAKE) --no-print-directory spin-wait-ready; then \
		$(MAKE) --no-print-directory dashboard-build >/dev/null || exit 1; \
		./scripts/tests/verify_served_dashboard_assets.sh http://127.0.0.1:3000 || exit 1; \
		$(MAKE) --no-print-directory seed-dashboard-data || exit 1; \
		SHUMA_BASE_URL=http://127.0.0.1:3000 SHUMA_API_KEY=$(SHUMA_API_KEY) SHUMA_FORWARDED_IP_SECRET=$(SHUMA_FORWARDED_IP_SECRET) ./scripts/tests/run_dashboard_e2e.sh --grep "adversary sim lane selector keeps off-state desired versus active truth and allows bot red team|red team tab renders bot red team browser-mode recent run evidence"; \
	else \
		echo "$(RED)❌ Error: Spin server not ready$(NC)"; \
		echo "$(YELLOW)   Start the server first: make dev$(NC)"; \
		exit 1; \
...
test-dashboard-e2e-adversary-sim: ## Run focused Playwright adversary-sim dashboard smoke checks
	...
	@if $(MAKE) --no-print-directory spin-wait-ready; then \
		...
		$(MAKE) --no-print-directory seed-dashboard-data || exit 1; \
		SHUMA_BASE_URL=http://127.0.0.1:3000 SHUMA_API_KEY=$(SHUMA_API_KEY) SHUMA_FORWARDED_IP_SECRET=$(SHUMA_FORWARDED_IP_SECRET) ./scripts/tests/run_dashboard_e2e.sh --grep "adversary sim (global toggle drives orchestration control lifecycle state|toggle emits fresh telemetry visible in monitoring raw feed|lane selector keeps off-state desired versus active truth and allows bot red team)"; \
	else \
		echo "$(RED)❌ Error: Spin server not ready$(NC)"; \
		echo "$(YELLOW)   Start the server first: make dev$(NC)"; \
		exit 1; \
