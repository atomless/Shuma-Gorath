...
ADVERSARIAL_RUNTIME_ARTIFACT_DIR ?= .spin/adversarial
...
ADVERSARY_SIM_SCRAPLING_PYTHON := $(call strip_wrapping_quotes,$(ADVERSARY_SIM_SCRAPLING_PYTHON))
ADVERSARY_SIM_SCRAPLING_SCOPE_DESCRIPTOR_PATH := $(call strip_wrapping_quotes,$(ADVERSARY_SIM_SCRAPLING_SCOPE_DESCRIPTOR_PATH))
ADVERSARY_SIM_SCRAPLING_SEED_INVENTORY_PATH := $(call strip_wrapping_quotes,$(ADVERSARY_SIM_SCRAPLING_SEED_INVENTORY_PATH))
ADVERSARY_SIM_SCRAPLING_CRAWLDIR := $(call strip_wrapping_quotes,$(ADVERSARY_SIM_SCRAPLING_CRAWLDIR))
...
DEPLOY_SHUMA_EVENT_LOG_IP_STORAGE_MODE := $(call prefer_process_env,SHUMA_EVENT_LOG_IP_STORAGE_MODE)
...
DEPLOY_ENV_ONLY := \
	...
	SHUMA_EVENT_LOG_IP_STORAGE_MODE="$(DEPLOY_SHUMA_EVENT_LOG_IP_STORAGE_MODE)" \
...
SPIN_ENV_ONLY_BASE := \
	...
	--env SHUMA_EVENT_LOG_IP_STORAGE_MODE=$(SHUMA_EVENT_LOG_IP_STORAGE_MODE) \
...
# Host-side wrappers need these env-only vars directly; Spin --env flags do not reach the supervisor processes.
SUPERVISOR_HOST_ENV := SHUMA_API_KEY="$(SHUMA_API_KEY)" SHUMA_FORWARDED_IP_SECRET="$(SHUMA_FORWARDED_IP_SECRET)" SHUMA_SIM_TELEMETRY_SECRET="$(SHUMA_SIM_TELEMETRY_SECRET)" SHUMA_ADVERSARY_SIM_SUPERVISOR_BASE_URL="$(ADVERSARY_SIM_SUPERVISOR_BASE_URL)" ADVERSARY_SIM_SCRAPLING_PYTHON="$(ADVERSARY_SIM_SCRAPLING_PYTHON)" ADVERSARY_SIM_SCRAPLING_SCOPE_DESCRIPTOR_PATH="$(ADVERSARY_SIM_SCRAPLING_SCOPE_DESCRIPTOR_PATH)" ADVERSARY_SIM_SCRAPLING_SEED_INVENTORY_PATH="$(ADVERSARY_SIM_SCRAPLING_SEED_INVENTORY_PATH)" ADVERSARY_SIM_SCRAPLING_CRAWLDIR="$(ADVERSARY_SIM_SCRAPLING_CRAWLDIR)"
...
dev: ## Build and run with file watching (auto-rebuild on save)
	...
	@./scripts/dev_watch_lock.sh cargo watch --poll -w src -w dashboard -w spin.toml $(DEV_WATCH_IGNORES) \
		...
		-s 'pkill -x spin 2>/dev/null || true; $(MAKE) --no-print-directory config-verify && $(MAKE) --no-print-directory dashboard-build >/dev/null 2>&1 && RUNTIME_INSTANCE_ID="$$(uuidgen)" && $(SUPERVISOR_HOST_ENV) SHUMA_ADVERSARY_SIM_AVAILABLE=$(DEV_ADVERSARY_SIM_AVAILABLE) SPIN_ALWAYS_BUILD=0 ./scripts/run_with_oversight_supervisor.sh spin up --direct-mounts $(SPIN_ENV_ONLY_BASE) $(SPIN_DEV_OVERRIDES) --env RUNTIME_INSTANCE_ID=$$RUNTIME_INSTANCE_ID --listen 127.0.0.1:3000'
...
run: ## Build once and run (no file watching)
	...
	@echo "$(YELLOW)❤️  Health:    http://127.0.0.1:3000/health$(NC)"
	@echo "$(YELLOW)🌀 Maze Preview: http://127.0.0.1:3000/admin/maze/preview (admin auth)$(NC)"
	@RUNTIME_INSTANCE_ID=$$(uuidgen); $(SUPERVISOR_HOST_ENV) SHUMA_ADVERSARY_SIM_AVAILABLE=$(DEV_ADVERSARY_SIM_AVAILABLE) ./scripts/run_with_oversight_supervisor.sh spin up $(SPIN_ENV_ONLY_BASE) $(SPIN_DEV_OVERRIDES) --env RUNTIME_INSTANCE_ID=$$RUNTIME_INSTANCE_ID --listen 127.0.0.1:3000
...
prod-start: ## Start production server using existing build artifacts and env (no build/config mutation)
	...
	@pkill -x spin 2>/dev/null || true
	@$(MAKE) --no-print-directory config-verify
	@RUNTIME_INSTANCE_ID=$$(uuidgen); $(SUPERVISOR_HOST_ENV) SHUMA_ADVERSARY_SIM_AVAILABLE=$(SHUMA_ADVERSARY_SIM_AVAILABLE) ./scripts/run_with_oversight_supervisor.sh spin up --from $(SPIN_UP_MANIFEST) $(SPIN_ENV_ONLY_BASE) $(SPIN_PROD_OVERRIDES) --env RUNTIME_INSTANCE_ID=$$RUNTIME_INSTANCE_ID --listen 0.0.0.0:3000
...
test: ## Run the canonical local/CI pre-merge suite: unit, env-lock contract, maze benchmark, Spin integration, adversarial matrix, SIM2 gates, and dashboard e2e (excludes live remote proofs)
	@echo "$(CYAN)============================================$(NC)"
	...
	@echo ""
	@echo "$(CYAN)Step 1/8: Rust Unit Tests + Env-Lock Contract$(NC)"
	@echo "$(CYAN)--------------------------------------------$(NC)"
	@./scripts/set_crate_type.sh rlib
	@cargo test || exit 1
	@$(MAKE) --no-print-directory test-rust-env-lock-contract || exit 1
	@echo ""
...
test-rust-env-lock-contract: ## Validate repo-wide Rust test env-lock discipline (no server required)
	@echo "$(CYAN)🧪 Running Rust env-lock contract checks...$(NC)"
	@python3 -m unittest scripts/tests/test_rust_env_lock_contract.py
...
test-verified-identity-target-contracts: ## Validate verified-identity Makefile selector and target-contract wiring
	@echo "$(CYAN)🧪 Running verified-identity target-contract checks...$(NC)"
...
test-oversight-host-impact: ## Run focused host-impact reconcile checks
	...
	@./scripts/set_crate_type.sh rlib
	@cargo test admin::oversight_reconcile::tests::primary_pressure_treats_latency_share_budget_miss_as_suspicious_origin_cost -- --exact --nocapture

test-host-impact-target-contracts: ## Validate host-impact Makefile selector and target-contract wiring
	@echo "$(CYAN)🧪 Running host-impact target-contract checks...$(NC)"
...
test-github-workflow-node24-majors: ## Verify GitHub workflows use the settled Node24-safe action majors
	@echo "$(CYAN)🧪 Running GitHub workflow Node24-major contract checks...$(NC)"
	@python3 -m unittest scripts/tests/test_github_workflow_node24_majors.py
...
test-deploy-linode: ## Validate Linode deploy-path helpers and production input gates
	...
	@$(MAKE) --no-print-directory test-prod-start-contract
...
test-prod-start-contract: ## Validate prod-start manifest selection and wrapper env handoff
	@echo "$(CYAN)🧪 Running prod-start contract verification...$(NC)"
...
test-adversarial-coverage-receipts: ## Run focused canonical category-coverage receipt checks across adversarial contracts and snapshot/benchmark gates
	...
	@cargo test observability::scrapling_owned_defense_surfaces::tests:: -- --nocapture
	@cargo test observability::operator_snapshot_live_traffic::tests:: -- --nocapture
	@cargo test observability::operator_snapshot::tests::snapshot_payload_projects_scrapling_owned_surface_gap_assignment -- --exact --nocapture
...
test-controller-action-surface: ## Run focused allowed-actions and controller-family mapping checks
	...
	@./scripts/set_crate_type.sh rlib
	@cargo test config::tests::allowed_actions_v1_exposes_conservative_controller_write_surface -- --exact --nocapture
	@cargo test config::tests::allowed_actions_v1_surfaces_auto_proposal_support_for_tunable_paths -- --exact --nocapture
	@cargo test config::tests::allowed_action_group_status_tracks_canonical_mutability_ring -- --exact --nocapture
	@cargo test config::tests::controller_config_family_for_patch_key_reuses_allowed_action_catalog -- --exact --nocapture

test-controller-action-surface-parity: ## Run focused controller mutability, escalation, and proposer parity checks
	@echo "$(CYAN)🧪 Running controller action-surface parity checks...$(NC)"
	@$(MAKE) --no-print-directory test-controller-action-surface
	@$(MAKE) --no-print-directory test-benchmark-comparison-contract

test-controller-mutability-policy: ## Run focused controller mutability policy checks
	@echo "$(CYAN)🧪 Running controller mutability policy checks...$(NC)"
	@./scripts/set_crate_type.sh rlib
	@cargo test config::tests::allowed_actions_v1_exposes_conservative_controller_write_surface -- --exact --nocapture
	@cargo test config::tests::controller_mutability_surface_classifies_expected_admin_and_objective_paths -- --exact --nocapture
	@cargo test config::tests::allowed_actions_v1_surfaces_group_and_family_mutability_from_canonical_policy -- --exact --nocapture
	@node --test \
		--test-name-pattern='admin config controller mutability groups match backend controller mutability policy' \
		e2e/dashboard.modules.unit.test.js

test-controller-hard-boundaries: ## Run focused controller hard-boundary classification checks
	@echo "$(CYAN)🧪 Running controller hard-boundary classification checks...$(NC)"
	@./scripts/set_crate_type.sh rlib
	@cargo test config::tests::controller_mutability_surface_classifies_expected_admin_and_objective_paths -- --exact --nocapture
	@cargo test config::tests::operator_objectives_are_never_mutable_and_not_patchable -- --exact --nocapture
	@cargo test admin::oversight_patch_policy::tests::hard_never_families_are_not_proposable_or_patch_matchable -- --exact --nocapture
	@cargo test admin::oversight_patch_policy::tests::mixed_families_reject_non_proposable_hard_boundary_keys -- --exact --nocapture

test-fingerprint-retention-cleanup: ## Run focused fingerprint retention cleanup checks for stale state and flow lifecycle
	@echo "$(CYAN)🧪 Running fingerprint retention cleanup checks...$(NC)"
	@./scripts/set_crate_type.sh rlib
	@cargo test signals::fingerprint::tests::stale_fingerprint_state_cleanup_evicts_other_identities -- --exact --nocapture
	@cargo test signals::fingerprint::tests::stale_fingerprint_flow_cleanup_evicts_old_buckets_but_keeps_current_window -- --exact --nocapture
	@cargo test signals::fingerprint::tests::fingerprint_cleanup_scan_is_cadence_gated -- --exact --nocapture

test-event-log-ip-storage-mode: ## Run focused event-log IP storage minimization checks
	@echo "$(CYAN)🧪 Running event-log IP storage minimization checks...$(NC)"
	@./scripts/set_crate_type.sh rlib
	@cargo test config::tests::validate_env_rejects_invalid_optional_event_log_ip_storage_mode -- --exact --nocapture
	@cargo test admin::api::tests::log_event_persists_masked_ip_when_event_log_ip_storage_mode_is_masked -- --exact --nocapture
	@cargo test admin::api::tests::log_event_persists_stable_pseudonymized_ip_when_event_log_ip_storage_mode_is_pseudonymized -- --exact --nocapture
	@cargo test admin::api::admin_config_tests::admin_monitoring_defaults_to_pseudonymized_view_and_supports_forensic_mode -- --exact --nocapture
	@cargo test admin::api::tests::admin_monitoring_forensic_mode_respects_masked_event_log_ip_storage_mode -- --exact --nocapture
	@cargo test admin::api::tests::security_privacy_payload_surfaces_event_log_ip_storage_mode_and_forensic_raw_availability -- --exact --nocapture
	@if ! command -v corepack >/dev/null 2>&1; then \
		echo "$(RED)❌ Error: corepack not found (install Node.js 18+).$(NC)"; \
		exit 1; \
	fi
	...
	@node --test \
		--test-name-pattern='runtime variable inventory meanings match writable and read-only admin config payload paths' \
		e2e/dashboard.modules.unit.test.js

test-dashboard-config-parity: ## Run focused dashboard config-parity tests for Advanced JSON and mutability classification
	@echo "$(CYAN)🧪 Running focused dashboard config parity checks...$(NC)"
	@if ! command -v corepack >/dev/null 2>&1; then \
		echo "$(RED)❌ Error: corepack not found (install Node.js 18+).$(NC)"; \
		exit 1; \
	fi
	...
	@node --test \
		--test-name-pattern='config form utils and JSON object helpers preserve parser contracts|advanced config template paths match writable admin config patch keys|admin config controller mutability groups match backend controller mutability policy|runtime variable inventory meanings match writable and read-only admin config payload paths' \
		e2e/dashboard.modules.unit.test.js
...
test-rsi-game-contract: ## Run focused recursive self-improvement game-contract checks
	@echo "$(CYAN)🧪 Running recursive self-improvement game-contract checks...$(NC)"
	@./scripts/set_crate_type.sh rlib
	@cargo test observability::operator_snapshot_objectives::tests::recursive_improvement_game_contract_names_rules_judge_and_legal_move_ring -- --exact --nocapture
	@cargo test config::tests::allowed_actions_v1_exposes_conservative_controller_write_surface -- --exact --nocapture
	@cargo test observability::operator_snapshot::tests::snapshot_payload_uses_persisted_objective_profile_and_typed_verified_identity_summary -- --exact --nocapture
	@cargo test admin::oversight_api::tests::manual_reconcile_route_records_observe_longer_when_classification_is_not_ready -- --exact --nocapture

test-rsi-scorecard: ## Run focused recursive self-improvement judge scorecard checks
	@echo "$(CYAN)🧪 Running recursive self-improvement judge scorecard checks...$(NC)"
	@./scripts/set_crate_type.sh rlib
	@cargo test observability::operator_snapshot_objectives::tests::recursive_improvement_game_contract_names_rules_judge_and_legal_move_ring -- --exact --nocapture
	@cargo test observability::operator_snapshot::tests::snapshot_payload_uses_persisted_objective_profile_and_typed_verified_identity_summary -- --exact --nocapture
	@cargo test admin::oversight_api::tests::manual_reconcile_route_records_observe_longer_when_classification_is_not_ready -- --exact --nocapture
...
test-oversight-episode-archive: ## Run focused recursive self-improvement episode archive checks
	...
	@./scripts/set_crate_type.sh rlib
	@cargo test observability::oversight_episode_archive::tests:: -- --nocapture
	@cargo test observability::benchmark_comparison::tests::candidate_snapshot_delta_reports_family_and_metric_progress -- --exact --nocapture
	@cargo test admin::oversight_api::tests::manual_reconcile_route_records_observe_longer_when_classification_is_not_ready -- --exact --nocapture
	@cargo test admin::oversight_api::tests::internal_agent_route_records_periodic_run_and_fail_closed_status_surface -- --exact --nocapture
	@cargo test admin::oversight_agent::tests::agent_cycle_keeps_canary_when_candidate_window_improves -- --exact --nocapture

test-oversight-move-selection-policy: ## Run focused oversight shortfall-attribution and move-selection checks
	@echo "$(CYAN)🧪 Running oversight move-selection policy checks...$(NC)"
	@./scripts/set_crate_type.sh rlib
	@cargo test observability::benchmark_results_comparison::tests:: -- --nocapture
	@cargo test admin::oversight_patch_policy::tests:: -- --nocapture
	@cargo test admin::oversight_reconcile::tests::reconcile_prefers_recommended_family_from_benchmark_guidance -- --exact --nocapture
...
test-supervisor-wrapper-contracts: ## Validate host-side adversary-sim and oversight wrapper wiring contracts (no server required)
	@echo "$(CYAN)🧪 Running supervisor wrapper contract checks...$(NC)"
...
test-adversarial-preflight: ## Validate adversarial required secrets and setup posture before runner execution
	...
	@python3 scripts/tests/adversarial_preflight.py --output "$(ADVERSARIAL_PREFLIGHT_REPORT)"

test-adversarial-artifact-contract: ## Validate adversarial/SIM2 generated receipt paths stay in untracked runtime artifact storage
	@echo "$(CYAN)🧪 Validating adversarial artifact path contract...$(NC)"
	@python3 -m unittest scripts/tests/test_adversarial_artifact_paths.py
...
test-adversary-sim-domain-contract: ## Run focused adversary-sim lifecycle and lane-domain checks without live runtime-surface traffic
	...
	@$(MAKE) --no-print-directory test-supervisor-wrapper-contracts
	@$(MAKE) --no-print-directory test-adversary-sim-target-contracts
...
test-adversary-sim-target-contracts: ## Validate adversary-sim Makefile selector and target-contract wiring
	@echo "$(CYAN)🧪 Running adversary-sim target-contract checks...$(NC)"
...
test-adversarial-fast: ## Run mandatory fast adversarial matrix (smoke + abuse + Akamai profiles)
	...
	@$(MAKE) --no-print-directory test-adversarial-artifact-contract || exit 1
...
test-sim2-realtime-bench: ## Run deterministic SIM2 realtime benchmark gate and emit latency/overflow/request-budget artifacts
	...
	@python3 scripts/tests/sim2_realtime_bench.py --output "$(SIM2_REALTIME_BENCH_REPORT)" --summary "$(SIM2_REALTIME_BENCH_SUMMARY)"
...
test-sim2-ci-diagnostics: ## Render SIM2 CI diagnostics artifact (timeline snapshots, event counts, refresh traces)
	...
	@python3 scripts/tests/render_sim2_ci_diagnostics.py --report scripts/tests/adversarial/latest_report.json --output "$(SIM2_CI_DIAGNOSTICS_REPORT)"
...
test-sim2-operational-regressions: ## Validate SIM2 operational regression diagnostics for active deterministic profiles (retention/cost/security required; failure/prod evaluated when present)
	...
	@python3 scripts/tests/check_sim2_operational_regressions.py --report scripts/tests/adversarial/latest_report.json --output "$(SIM2_OPERATIONAL_REGRESSIONS_REPORT)" --allow-missing-domain failure_injection --allow-missing-domain prod_mode_monitoring --min-large-payload-samples-for-compression-check 2
...
test-sim2-operational-regressions-strict: ## Validate all SIM2 operational regression domains with strict missing-domain and compression enforcement
	...
	@python3 scripts/tests/check_sim2_operational_regressions.py --report scripts/tests/adversarial/latest_report.json --output "$(SIM2_OPERATIONAL_REGRESSIONS_REPORT)"
...
env-help: ## Show supported env-only runtime overrides
	...
	@echo "  SHUMA_EVENT_LOG_IP_STORAGE_MODE"
