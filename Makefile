...
test-dashboard-monitoring-accountability: ## Run focused dashboard Monitoring machine-contract accountability checks
	...
	@$(MAKE) --no-print-directory test-dashboard-svelte-check
	@node --test \
		--test-name-pattern='dashboard monitoring accountability adapters normalize benchmark and oversight payloads safely|dashboard monitoring tab wires benchmark and oversight machine contracts through state and refresh runtime' \
		e2e/dashboard.modules.unit.test.js
	@if $(MAKE) --no-print-directory spin-wait-ready; then \
		$(MAKE) --no-print-directory seed-dashboard-data || exit 1; \
...
test-dashboard-verified-identity-pane: ## Run focused Verification-tab verified-identity surfacing checks
	...
	@$(MAKE) --no-print-directory test-dashboard-svelte-check
	@node --test \
		--test-name-pattern='dashboard verification tab wires verified identity operator snapshot and store state|dashboard config tabs reuse shared panels, save flows, and owned controls|dashboard refresh runtime owns bounded cache, delta, and red-team monitoring refresh flows' \
		e2e/dashboard.modules.unit.test.js
	@if $(MAKE) --no-print-directory spin-wait-ready; then \
		$(MAKE) --no-print-directory seed-dashboard-data || exit 1; \
