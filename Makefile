...
test-monitoring-telemetry-contract: ## Run focused external-only monitoring telemetry checks
	...
	@cargo test admin::api::tests::handle_admin_monitoring_delta_keeps_freshness_anchor_when_page_is_empty -- --exact --nocapture
	@cargo test admin::api::tests::handle_admin_monitoring_delta_reads_only_requested_page_of_event_values -- --exact --nocapture
	@cargo test admin::api::tests::handle_admin_monitoring_stream_resumes_from_last_event_id -- --exact --nocapture
	@cargo test admin::api::tests::handle_admin_monitoring_stream_orders_event_ids_across_reconnects -- --exact --nocapture
	@$(MAKE) --no-print-directory test-dashboard-unit
