...
4. That operator-surface cleanup is no longer the immediate mainline. The post-stance-model execution order is now `STANCE-MODEL-1`, then full-power Scrapling in `SIM-SCR-FULL-1`, then repeated strict `human_only_private` operational proof in `RSI-GAME-HO-1`, then the remaining LLM attacker runtime proof in `SIM-LLM-1C3`, then a second strict-baseline proof in `RSI-GAME-HO-2`, and only after that the later `humans_plus_verified_only` sweep.
