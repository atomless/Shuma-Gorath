...
5. The immediate next execution priority inside that sequence is `SIM-REALISM-1A`, followed by `SIM-REALISM-1B`, `SIM-REALISM-1C`, and `SIM-REALISM-1D`.
...
8. The next adversary-side truth gap is no longer route knowledge or category ownership. It is behavior realism: both Scrapling and the later LLM lane now need explicit cadence, dwell, burst, and identity or session contracts before mixed-attacker or tuning claims can be called representative.
