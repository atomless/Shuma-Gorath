...
2. Do not reopen `TUNE-SURFACE-2`, `TUNE-SURFACE-2A`, `TUNE-SURFACE-2B`, `TUNE-BAN-1`, or `TUNE-BAN-2` while `SIM-REALISM-2F..2J` remain open.
3. The reason is now explicit: Tuning must optimize against representative attacker pressure, and that representation still falls short on traversal depth, frontier truth, richer root-host public terrain, and trusted-ingress identity realism.
...
10. The next adversary-side truth gap is no longer route knowledge or category ownership. It is behavior realism across two layers: first local cadence or dwell or burst contracts, then field-grounded pressure, identity, transport, browser-background, recurrence, traversal-frontier, and trusted-ingress identity realism.
