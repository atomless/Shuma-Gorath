...
6. `SIM-SH-SURFACE-1`, `SIM-SCR-FIT-1`, `SIM-SCR-FIT-2`, `SIM-SCR-COVER-2`, `VID-TAX-1`, `VID-TAX-2`, `VID-BOT-1`, `VID-GUARD-1`, and `SIM-LLM-FIT-1` in [`../../todos/todo.md`](../../todos/todo.md)
...
3. No further telemetry architecture sweep is required before `MON-OVERHAUL-1`; the remaining pre-Monitoring work is now the adversary-side truth follow-on that expands Scrapling's request-native category ownership and receipt-backed coverage, plus the verified-identity calibration follow-on that turns Web Bot Auth and other verified traffic into faithful taxonomy truth and closed-loop guardrails before the human projection is redesigned around those settled semantics.
