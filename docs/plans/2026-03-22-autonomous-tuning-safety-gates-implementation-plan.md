...
That gate now has earlier prerequisites:

9. canonical non-human taxonomy,
10. bounded category-classification confidence over both simulated and observed traffic,
11. explicit lane-fulfillment coverage for the target categories.
...
Codify the canonical non-human traffic taxonomy.
...
Codify category-classification confidence and evidence receipts across both simulated and observed traffic.
...
Codify the category-to-lane fulfillment matrix.

Must include:

1. explicit mapping from each target category to the lane or mode intended to fulfill it,
2. Scrapling fulfillment where possible,
3. frontier or containerized LLM fulfillment where Scrapling is not the right fit,
4. explicit gaps when a category still lacks a credible fulfillment path.
...
1. the protected non-human category taxonomy derived from the canonical category model,
