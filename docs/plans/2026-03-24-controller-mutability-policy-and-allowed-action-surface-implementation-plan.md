...
4. Keep `MON-OVERHAUL-1` independent of this work where possible, but require this mutability policy before any later reopened botness or fingerprint control consolidation, before any future controller-explanation expansion in `Tuning`, and before the later recursive phases `OVR-AGENT-2` and `OVR-CODE-1`.
