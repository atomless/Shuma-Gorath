...
That operator-surface follow-on is no longer the immediate mainline. The active execution order has now moved through attacker-faithful Scrapling (`SIM-SCR-CHALLENGE-2A..2D`) and the request-native `geo_ip_policy` source-diversification follow-on (`SIM-SCR-GEO-1`); browser or stealth Scrapling remains conditional only where the owned-surface matrix later proves request-native plus public-network identity diversity is still insufficient. With the request-native Scrapling-owned surface matrix now receipt-backed end to end, the mainline proceeds into the already-landed legal move ring (`CTRL-SURFACE-1..3`), then the first judge-side game-contract slices (`RSI-GAME-1A`, `RSI-GAME-1B`, `RSI-SCORE-1`, `RSI-GAME-1C`), and only then the first explicit self-improving loop before returning to deferred dashboard follow-ons. That first explicit loop is now defined more tightly: it must prove the bounded oversight loop is operating over the Scrapling attacker basis, that post-sim oversight execution is linked to that attacker evidence, and that the episode archive records coherent lineage for the resulting move rather than leaving those truths scattered across separate proof tranches.
