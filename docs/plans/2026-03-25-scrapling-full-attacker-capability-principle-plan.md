...
**Goal:** Reframe the Scrapling planning chain so Shuma adopts attacker-relevant upstream Scrapling capability by default for Scrapling-owned surfaces, with explicit exclusions rather than default omission.

**Architecture:** Preserve the already-landed truthful request-native Scrapling baseline, but change the governing rule for what comes next. Add an active capability-matrix and omission-ledger tranche, keep broader browser or stealth Scrapling as a bounded follow-on for Scrapling-owned surfaces where the matrix says it belongs, and keep `automated_browser` ownership as a separate later question rather than letting it blur into every wider Scrapling capability discussion.
...
3. Do not let browser or stealth Scrapling adoption silently broaden category ownership beyond the already-ratified boundaries.
...
**Work:**
1. Replace the conservative "only reopen if a later gap proves it" framing with the stronger default-adoption principle for attacker-relevant upstream Scrapling capability.
...
3. State explicitly that every omitted attacker-relevant capability must have a recorded reason.
...
2. Reframe `SIM-SCR-CHALLENGE-2C` so it is no longer blocked only because request-native coverage exists; instead, it should wait on the active capability-matrix tranche and then implement the broader Scrapling-owned browser or stealth subset that the matrix requires.
3. Keep `SIM-SCR-BROWSER-1` separate for the later truthful `automated_browser` question.
...
**Acceptance criteria:**
1. The backlog contains one active Scrapling capability-expansion driver rather than only conditional blocked follow-ons.
