...
- Those local contributor flows may still leave `SHUMA_GATEWAY_UPSTREAM_ORIGIN` set in `.env.local`; with local contributor ingress enabled, unknown public-root misses now stay local and return local `404`s instead of trying to forward to the remote upstream.
