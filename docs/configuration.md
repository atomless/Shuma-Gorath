...
| `SHUMA_VERIFIED_IDENTITY_ENABLED` | `true` | Enables the verified-identity subsystem. The default is now on so Shuma evaluates verified identity signals from the start while the configured non-human traffic stance still remains the deciding authorization policy. |
...
| `SHUMA_VERIFIED_IDENTITY_NON_HUMAN_TRAFFIC_STANCE` | `deny_all_non_human` | Legacy verified-identity override input for authenticated non-human traffic (`deny_all_non_human`, `allow_only_explicit_verified_identities`, `allow_verified_by_category`, `allow_verified_with_low_cost_profiles_only`). Verification must not be treated as unconditional allow. Until `STANCE-MODEL-1B` lands, Shuma still resolves verified-identity request-path decisions from this input while `operator_snapshot_v1.effective_non_human_policy` exposes the resulting conflict against the canonical category posture matrix. |
...
- `operator_snapshot_v1.non_human_stance_presets` and `operator_snapshot_v1.effective_non_human_policy` are the first machine-first bridge away from the old dual-stance model. They do not yet rebase runtime authorization, but they do make the current preset choices and verified-identity conflict visible to later Game Loop and Tuning work instead of leaving the mismatch implicit.
- `verified_identity.non_human_traffic_stance` now directly feeds the bounded `beneficial_non_human_posture` benchmark family. When verified identity is disabled that family reports `not_applicable`; when it is enabled the benchmark compares observed verified-identity tranche outcomes and verification coverage against the configured stance, and it now also exposes bounded verified-identity calibration metrics for taxonomy alignment mismatch, verified-versus-botness conflict pressure, and user-triggered-agent friction mismatch.
