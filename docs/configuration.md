...
| `SHUMA_VERIFIED_IDENTITY_ENABLED` | `true` | Enables the verified-identity subsystem. The default is now on so Shuma evaluates verified identity signals from the start while explicit verified-identity overrides still remain deny-first unless operators configure named policies or category defaults. |
...
- `operator_snapshot_v1.non_human_stance_presets` now exposes the canonical preset catalog and matched preset id for the current objective matrix plus verified-identity mode. The first catalog includes `balanced_default`, `human_only_private`, and `humans_plus_verified_only`.
- `operator_snapshot_v1.effective_non_human_policy` now materializes the resolved machine-first policy view row by row: base category posture from `operator_objectives_v1.category_postures`, the current verified-identity override status for categories that can authenticate, the resulting effective verified-identity action or posture when it is deterministic, and the source-of-authority lineage that produced that result.
...
- `verified_identity.category_defaults` are explicit verified-identity overrides. They are consulted after named verified-identity policies and before the default verified-identity deny fallback.
- If no named policy or eligible category default matches, verified identity falls back to deny.
...
- `operator_snapshot_v1.non_human_stance_presets` and `operator_snapshot_v1.effective_non_human_policy` now expose the canonical stance presets plus the resolved verified-identity override layer. Runtime verified-identity authorization now follows explicit overrides rather than a second top-level stance, while benchmark and later Game Loop consumers still complete their migration in the later alignment tranche.
- `verified_identity.override_mode` now feeds the bounded `beneficial_non_human_posture` benchmark family. When verified identity is disabled that family reports `not_applicable`; when it is enabled the benchmark compares observed verified-identity tranche outcomes and verification coverage against the derived explicit-override mode, and it now also exposes bounded verified-identity calibration metrics for taxonomy alignment mismatch, verified-versus-botness conflict pressure, and user-triggered-agent friction mismatch.
