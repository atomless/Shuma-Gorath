...
| `SHUMA_ADVERSARY_SIM_DURATION_SECONDS` | `180` | Run-window duration for control-triggered adversary simulation orchestration. Value must be between `30` and `900` seconds (inclusive). Runtime-dev keeps the supervisor-owned post-canary candidate follow-on run at `30` seconds so local judged cycles can materialize candidate evidence quickly without silently changing the general configured bound. |
...
| `SHUMA_VERIFIED_IDENTITY_ENABLED` | `true` | Enables the verified-identity subsystem. The default is now on so Shuma evaluates verified identity signals from the start while the canonical operator-objectives stance remains the deciding non-human policy authority. |
...
- Core: `shadow_mode`, `rate_limit`, `ban_duration`, `ban_durations.{honeypot,ip_range_honeypot,maze_crawler,rate_limit,admin,cdp,edge_fingerprint,tarpit_persistence,not_a_bot_abuse,challenge_puzzle_abuse}`, `honeypot_enabled`, `honeypots`, `browser_policy_enabled`, `browser_block`, `browser_allowlist`, `bypass_allowlists_enabled`, `allowlist`, `path_allowlist_enabled`, `path_allowlist`, `ip_range_policy_mode`, `ip_range_emergency_allowlist`, `ip_range_custom_rules`, `js_required_enforced`.
...
- Verified identity: `verified_identity.{enabled,native_web_bot_auth_enabled,provider_assertions_enabled,replay_window_seconds,clock_skew_seconds,directory_cache_ttl_seconds,directory_freshness_requirement_seconds,named_policies,category_defaults,service_profiles}`.
...
- Shuma now defines one canonical controller mutability policy over these surfaces:
  ...
  - `controller_tunable`: bounded sensitivity and rollout controls such as `js_required_enforced`, PoW, challenge, not-a-bot, botness thresholds and weights, bounded maze rollout posture, bounded CDP rollout thresholds, and bounded fingerprint sensitivity caps.
...
- `replay_promotion.summary` remains the bounded replay-lineage provenance contract. It still distinguishes `not_materialized`, `advisory_only`, and `protected` replay states, but it is no longer the only protected-evidence basis the controller can consume.
...
- The deciding non-human stance now comes from `operator_objectives_v1.category_postures` and the derived `effective_non_human_policy_v1` projection, not from a separate verified-identity top-level stance field.
- `verified_identity.effective_non_human_policy.profile_id=human_only_private` means verified identity is evidence only. Named policies and category defaults do not override the canonical category posture in that strict baseline.
- `verified_identity.effective_non_human_policy.profile_id=humans_plus_verified_only` keeps the same canonical category posture rows but changes `verified_identity_override_mode` to `explicit_overrides_eligible`, which allows named policies and category defaults to act as explicit overrides.
- If no named policy or eligible category default matches when explicit overrides are eligible, verified identity falls back to the canonical category posture for the crosswalked non-human category instead of consulting a second stance system.
- Verified identity now crosswalks into the seeded canonical non-human taxonomy before machine-first snapshot and benchmark projection: `search -> indexing_bot`, `training -> ai_scraper_bot`, `preview -> http_agent`, `service_agent -> http_agent`, `user_triggered_agent -> agent_on_behalf_of_human`, and `other -> verified_beneficial_bot`.
- The bounded `beneficial_non_human_posture` benchmark family now reads the resolved effective policy contract instead of the removed `verified_identity.non_human_traffic_stance` field. When verified identity is disabled that family reports `not_applicable`; when it is enabled the benchmark compares observed verified-identity tranche outcomes and verification coverage against the resolved profile plus override mode, and it also exposes bounded verified-identity calibration metrics for taxonomy alignment mismatch, verified-versus-botness conflict pressure, and user-triggered-agent friction mismatch.
- Verified-identity snapshot projection now includes bounded `taxonomy_alignment` receipts and counts so later Monitoring can show whether verified category claims aligned with the canonical taxonomy, fell back through `other`, or currently lack corroborating live classification evidence.
- Diagnosis now treats repeated verified-identity calibration problems as no-harm guardrails. When tolerated or allowed verified traffic is being short-circuited, when user-triggered verified agents are experiencing human-like friction mismatch, or when taxonomy alignment is degraded, tuning eligibility is blocked and reconcile fails closed to `observe_longer`; it does not auto-allow or silently tune through the conflict.
