...
| `SHUMA_LOCAL_CONTRIBUTOR_INGRESS_ENABLE` | No | `false` | Local contributor runtime switch for self-contained trusted-ingress browsing. When `true`, non-`/shuma/*` public-path misses stay local and return local `404`s instead of falling through to `SHUMA_GATEWAY_UPSTREAM_ORIGIN`. This must only be enabled for contributor-local trusted-ingress runs and must not be used as a deployment shortcut. |
