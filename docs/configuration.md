...
| `ADVERSARY_SIM_SCRAPLING_PUBLIC_NETWORK_IDENTITIES` | No | empty | Optional JSON array of bounded public-network identities available to the repo-owned Scrapling worker. Each entry must include `identity_id`, must set `identity_class` to `http_proxy`, must include an `http` or `https` `proxy_url`, and may include `expected_geo_country`. This env-only pool is the attacker-faithful request-native source-diversification contract for `geo_ip_policy`; it is not admin-writable and must not use privileged Shuma-only geo hints. |
...
- `game_contract_v1.judge_scorecard` now makes the judge explicit by partitioning optimization targets, hard guardrails, regression anchors, explanatory diagnostics, and homeostasis inputs over the existing machine-first benchmark and guardrail surfaces.
