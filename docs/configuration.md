...
- Later verified-identity calibration tranches add alignment receipts, verified-identity versus botness conflict metrics, and fail-closed diagnosis guardrails. The current landed behavior is the faithful category projection itself, not yet the later mismatch or no-harm controller logic.
