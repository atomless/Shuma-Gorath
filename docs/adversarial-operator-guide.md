...
`SIM-REALISM-2I` now adds the first truthful client-IP restoration path for those degraded cases without widening attacker privileges: when `SHUMA_FORWARDED_IP_SECRET` is configured, the adversary-sim supervisor can launch or consume a Shuma-owned local trusted-ingress proxy, thread its URL into Scrapling and Agentic worker plans as ordinary proxy configuration, and let the existing runtime forwarded-header trust gate restore parseable client IPs. The workers themselves still must not emit `x-shuma-forwarded-secret` or any equivalent privileged header. If the trusted-ingress proxy is unavailable, the run must stay explicitly degraded rather than silently pretending local churn is real network identity. The focused proof for that contract is `make test-adversary-sim-trusted-ingress-ip-realism`.
`SIM-REALISM-2J` now carries that truth through the observer surfaces as well: recent-run monitoring history and operator-snapshot read models preserve a compact `identity_provenance_mode`, and Red Team plus Game Loop now render states such as `Trusted ingress IP observed`, `Pool-backed proxy identity`, `Bucketed identity only`, or `Degraded local identity` instead of leaving operators to infer realism from `unknown` or hashed bucket labels. The focused proof for that slice is `make test-adversary-sim-identity-observer-truth`.
