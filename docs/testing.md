...
make test-setup-runtime-bootstrap # Validate setup-runtime bootstrap installers, including repo-owned Scrapling browser provisioning
...
make test-adversarial-llm-runtime-proof # Focused bounded LLM runtime proof gate (dispatch + recent-run/operator-snapshot closure)
...
- `make test-dashboard-scrapling-evidence` is the narrow proof path for receipt-backed Scrapling attack evidence surfacing; it covers bounded recent-run adaptation of fulfillment modes, category coverage, exercised surfaces, expected pass or fail receipt summaries, and owned-surface receipts plus rendered proof that `Red Team` carries the detailed evidence while `Game Loop` only carries compact corroboration.
- `make test-dashboard-red-team-llm-runtime` is the narrow proof path for `bot_red_team` browser-mode operator visibility; it covers the enabled lane selector state plus rendered recent-run evidence for browser-mode fulfillment without dragging in broader Game Loop or monitoring surface work.
...
- `make test-adversarial-llm-runtime-proof` is the focused proof path for `SIM-LLM-1C3`; it extends the dispatch gate with the bounded recent-run receipt summary, `bot_red_team` mode/category normalization, and operator-snapshot projection needed to prove the live LLM runtime is visible end to end through the machine-first recent-run path.
- `make test-adversarial-llm-browser-runtime` is the focused proof path for `SIM-LLM-BROWSER-1`; it extends the live `bot_red_team` contract with container browser-action execution, recent-run receipt projection, and rendered Red Team proof that browser-mode evidence is operator-visible end to end.
...
- `make test-rsi-game-human-only-mixed-baseline` is the focused proof path for `RSI-GAME-HO-2A`; it proves the first mixed-attacker strict-baseline seam where a retained Scrapling-driven `human_only_private` cycle is followed by a later `bot_red_team` post-sim canary on that retained config while both lanes remain visible in recent-run truth.
...
- `make test-setup-runtime-bootstrap` - focused runtime-bootstrap gate covering repo-owned Scrapling venv creation, Playwright browser provisioning, and fail-closed readiness when the selected browser executable is missing
- `make test-adversary-sim-scrapling-worker` - focused real Scrapling lane gate covering the internal beat/result contract, bounded crawler plus direct-request persona execution, first browser-backed not-a-bot and puzzle challenge interactions, runnable dynamic or stealth runtime contract, mode-specific signed sim telemetry on real requests, host-side supervisor source-contract wiring, and the supervisor's HTTP transport parser contract
...
- `make test-setup-runtime-bootstrap` proves the repo-owned Scrapling runtime provisions the selected Playwright browser and fails readiness closed if the browser executable is missing.
- `make test-adversary-sim-scrapling-worker` proves the hosted worker lane itself, including host-side supervisor parsing, fail-closed worker result shaping, runnable browser-session support for dynamic and stealth Scrapling, and the first browser-backed not-a-bot and puzzle owned-surface interactions.
...
- `make test-operator-objectives-contract` is the focused objective-profile and decision-evidence gate for persisted `operator_objectives_v1`, the operator-objectives admin endpoint, the bounded decision ledger, snapshot wiring, the canonical non-human stance preset catalog, and the resolved `effective_non_human_policy` snapshot section introduced by `STANCE-MODEL-1A`.
