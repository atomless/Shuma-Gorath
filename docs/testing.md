...
make test-adversary-sim-target-contracts # Explicit adversary-sim Makefile selector/target-contract lane
...
make test-verified-identity-target-contracts # Explicit verified-identity Makefile selector/target-contract lane
...
make test-host-impact-target-contracts # Explicit host-impact Makefile selector/target-contract lane
...
- `make test-adversary-sim-target-contracts` - explicit Makefile selector and wiring lane for adversary-sim lifecycle, lane-selection, and Scrapling category-fit proof targets
...
- `make test-verified-identity-target-contracts` - explicit Makefile selector and wiring lane for verified-identity calibration, alignment, conflict-metric, and guardrail proof targets
...
- `make test-host-impact-target-contracts` - explicit Makefile selector and wiring lane for the host-impact telemetry, benchmark, and reconcile proof targets
...
- `make test-adversary-sim-target-contracts` proves the adversary-sim Makefile selector and target wiring contract for the lifecycle and Scrapling category-fit proof lanes.
...
- `make test-verified-identity-calibration-readiness` is the focused bridge gate before `VID-TAX-1` through `VID-GUARD-1`; it proves the current verified-identity taxonomy, snapshot, benchmark, and reconcile seams without over-claiming future alignment or conflict behavior. Selector-only Makefile composition proof now lives in `make test-verified-identity-target-contracts`.
...
- `make test-verified-identity-target-contracts` is the explicit selector and wiring lane for those verified-identity make targets; it proves the feature targets keep the intended cargo selectors without reintroducing selector-only proof into the behavior gates.
...
- `make test-oversight-post-sim-trigger` is the focused post-sim oversight gate for completed-run trigger detection, bounded replay-once semantics for a finished sim run, and post-sim agent execution truth. Shared shell-wrapper archaeology now lives in the explicit supervisor wrapper contract lane instead of this feature gate.
...
- `make test-adversary-sim-domain-contract` is the focused backend adversary-sim lifecycle and lane-domain gate that stays off the live runtime-surface path. It now includes the explicit `make test-adversary-sim-target-contracts` lane instead of burying selector-only proof inside lifecycle or Scrapling category-fit behavior targets.
...
- `make test-supervisor-wrapper-contracts` is the explicit host-side wrapper wiring lane. It keeps the remaining shell/source-shape checks for `scripts/run_with_adversary_sim_supervisor.sh` and `scripts/run_with_oversight_supervisor.sh` in one contract target instead of hiding them inside feature-behavior gates.
