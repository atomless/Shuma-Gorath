...
make test-dashboard-red-team-pane # Focused Red Team controls and recent-run surfacing checks
...
- `make test-dashboard-red-team-pane` is the narrow proof path for the leaner `Red Team` tab; it covers adversary-sim status adaptation, runtime normalization, shared control wiring, and the rendered recent-run contract without keeping retired status-truth or Scrapling-detail panels alive.
- `make test-dashboard-game-loop-accountability` is the narrow proof path for the observer-facing `Game Loop`; it covers operator/oversight adapter truth, bounded snapshot wiring through the refresh runtime, durable observer-round lineage, missing-receipt honesty, and rendered Playwright proof that the tab stays scoped to recent rounds plus adversary and defence casts rather than the retired lower machine-diagnostic furniture.
