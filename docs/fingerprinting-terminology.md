...
| `Akamai Bot Signal` | Normalized bot-risk outcomes ingested from Akamai-shaped edge payloads (`/fingerprint-report`) when enabled in Akamai edge posture. | Edge-origin confidence input that can augment or dominate selected local decisions by mode. | Not direct browser-runtime CDP introspection; non-edge deployments keep the operator controls hidden. |
