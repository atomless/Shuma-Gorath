...
- `POST /admin/oversight/reconcile` - Run one bounded recommend-only oversight reconcile cycle over the already-materialized machine-first snapshot. Returns the pure reconcile result, config-validation outcome for any proposed patch, and the persisted `oversight_decision_ledger_v1` row. This endpoint never writes runtime config in this tranche; if `operator_snapshot_v1` is missing it returns an explicit `insufficient_evidence` decision instead of mutating or repairing state on read.
- `GET /admin/oversight/history` - Read the bounded persisted `oversight_decision_ledger_v1` history used by the first agent loop. Returns recent recommend-only proposals and refusals, including trigger source, benchmark context, validation status, refusal reasons, and compact evidence references.
- `GET /admin/oversight/agent/status` - Read the bounded shared-host agent status projection for the first recommend-only loop. Returns the execution-boundary contract, periodic and post-sim trigger semantics, the latest agent run, recent bounded run history, and the latest linked oversight decision row so later Monitoring can project the backend loop without inventing its own semantics.
