...
- `GET /admin/oversight/history` - Read the bounded persisted `oversight_decision_ledger_v1` history used by the first closed config loop. Returns recent preview/apply/watch/rollback decisions, including trigger source, benchmark context, validation status, refusal reasons, compact evidence references, the persisted `apply.stage` lineage (`eligible`, `canary_applied`, `watch_window_open`, `improved`, `refused`, or `rollback_applied`), and the current `game_contract_v1` so history consumers can tell which immutable rules, judge scorecard partitions, and legal move ring the loop is operating under.
- `GET /admin/oversight/agent/status` - Read the bounded shared-host agent status projection for the first bounded canary-apply loop. Returns the execution-boundary contract, periodic and post-sim trigger semantics, the latest agent run, recent bounded run history, and the latest linked oversight decision row, including `execution.apply.stage` so later Monitoring can project the backend loop without inventing its own semantics.
