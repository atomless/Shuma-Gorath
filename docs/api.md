...
- `GET /admin/oversight/history` - Read the bounded persisted `oversight_decision_ledger_v1` history used by the first closed config loop. Returns the same canonical `recursive_improvement_game_contract_v1` summary projected by `operator_snapshot_v1`, plus recent preview/apply/watch/rollback decisions, including trigger source, benchmark context, validation status, refusal reasons, compact evidence references, and the persisted `apply.stage` lineage (`eligible`, `canary_applied`, `watch_window_open`, `improved`, `refused`, or `rollback_applied`).
