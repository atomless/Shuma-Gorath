...
- Request-outcome scope, lane, and category summaries now also carry bounded `forwarded_upstream_latency_ms_total` counters for forwarded requests. The closed loop uses those totals to derive `suspicious_forwarded_latency_share`, a host-impact proxy that complements the older forwarded request-rate and byte-rate views without inventing speculative CPU or billing units.
