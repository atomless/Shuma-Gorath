...
- Dashboard DOM-composition mandate (strict): whenever adding or modifying dashboard tab or panel markup, agents MUST keep the rendered DOM hierarchy as clean and shallow as the canonical pattern allows. Agents MUST NOT stack section wrappers, panel chrome, headings, or bordered containers unless each layer has a distinct semantic responsibility that is visible in the rendered result.
