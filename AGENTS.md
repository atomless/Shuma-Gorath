...
- Dashboard DOM-composition mandate (strict): whenever adding or modifying dashboard tab or panel markup, agents MUST keep the rendered DOM hierarchy as clean and shallow as the canonical pattern allows. The problem is needless nesting, not nesting itself: agents MAY nest wrappers when each layer has a distinct semantic or structural responsibility that is visible in the rendered result, and MUST NOT add extra section wrappers, panel chrome, headings, or bordered containers when they do not own a real job.
- Dashboard whole-DOM review mandate (strict): agents MUST NOT build or edit dashboard DOM locally in isolation. Before claiming a dashboard UI slice is complete, agents MUST step back and inspect the whole rendered tab or panel DOM structure so they understand how the new markup interacts with surrounding section shells, shared wrappers, borders, headings, and spacing. If the wider structure has not been inspected, the slice is incomplete.
- Dashboard section-ownership mandate (strict): agents MUST NOT leave duplicate titles, nested section-inside-section shells, empty heading wrappers, border-only containers whose only child is another full panel/section using the same chrome, or repeated divider lines caused by stacked wrappers. If a subsection does not need its own heading or border, agents must remove that wrapper or extend the shared primitive so the extra shell is not rendered.
...
17. Non-negotiable completion proof for large tranches/refactors (release-blocking):
   ...
   - agents MUST NOT claim that a tranche has "landed" unless they have directly observed every acceptance criterion that is either explicitly written for that tranche or plainly implied by the user-visible goal of the slice; narrow surrogate proofs are insufficient when the obvious runtime or operator-facing acceptance check was not observed,
   - if a test target or local source-contract proof passes but a live, rendered, or operator-facing acceptance check remains unobserved, ambiguous, or contradictory, the tranche remains open and MUST be reported as incomplete,
   - when observed evidence contradicts an earlier completion claim, agents MUST correct the record in plans/TODOs/completion history before proceeding, then reopen the tranche and continue until the contradicted acceptance criteria are truly satisfied,
