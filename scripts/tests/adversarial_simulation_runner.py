...
def compute_frontier_diversity_scoring(
    ...
    *,
    ...
    novelty_avg = (
        sum(novelty_scores) / float(len(novelty_scores)) if novelty_scores else 0.0
    )
    ...
    behavioral_coverage = (
        len(generated_behavioral) / float(len(target_behavioral))
        if target_behavioral
        else 0.0
    )
...
def compute_frontier_discovery_quality_metrics(
    ...
    *,
    ...
    if total_provider_slots > 0:
        provider_outage_impact = (
            (total_provider_slots - configured) * 100.0
        ) / float(total_provider_slots)
    else:
...
def build_attack_plan(
    ...
    if not bool(constraints.get("deterministic_seed_required", True)):
        raise SimulationError(
            "frontier attack-generation contract must keep deterministic_seed_required=true"
        )
    ...
    def add_candidate(
        *,
        ...
        try:
            sanitized_payload = sanitize_frontier_payload(raw_payload)
        except Exception as exc:
    ...
    seed_candidate_count = len(
        [row for row in candidates if str(row.get("generation_kind") or "") == "seed"]
    )
