...
class SimPublicBuildFlowContractTests(unittest.TestCase):
    ...
    def test_contributor_dev_flows_use_stale_refresh_not_forced_rebuild(self) -> None:
        dev_body = target_body("dev")
        run_body = target_body("run")
        self.assertIn("sim-public-refresh-if-stale", dev_body)
        self.assertIn("sim-public-refresh-if-stale", run_body)

    def test_runtime_only_flows_do_not_generate_contributor_site(self) -> None:
