...
fn telemetry_field_classification_schema() -> serde_json::Value {
    json!([
        ...
        {
            "field": "event.llm_runtime_summary.fulfillment_mode",
            "class": "internal",
            "persistence": "allow"
        },
        {
            "field": "event.llm_runtime_summary.category_targets",
            "class": "internal",
            "persistence": "allow"
        },
        {
            "field": "event.llm_runtime_summary.generation_source",
            "class": "internal",
            "persistence": "allow"
        },
        {
            "field": "event.llm_runtime_summary.provider",
            "class": "internal",
            "persistence": "allow"
        },
        {
            "field": "event.llm_runtime_summary.model_id",
            "class": "internal",
            "persistence": "allow"
        },
        {
            "field": "event.llm_runtime_summary.latest_action_receipts.path",
            "class": "internal",
            "persistence": "allow"
        },
        {
            "field": "event.llm_runtime_summary.latest_action_receipts.status",
            "class": "internal",
            "persistence": "allow"
        },
        {
...
mod tests {
    ...
    #[test]
    fn recent_sim_run_history_projects_llm_runtime_receipts_and_categories() {
        let store = MockStore::new();
        let now = now_ts();
        ...
        persist_event_record(
            ...
            EventLogRecord {
                ...
                outcome_code: None,
                botness_score: None,
                sim_run_id: Some("simrun-llm-runtime-proof".to_string()),
        ...
        let row = recent_runs
            .iter()
            .find(|value| value.run_id == "simrun-llm-runtime-proof")
            .expect("llm runtime row");
        ...
        let llm_runtime_summary = row
            .llm_runtime_summary
            .as_ref()
            .expect("llm runtime summary");
        assert_eq!(llm_runtime_summary.backend_kind, "frontier_reference");
        ...
        assert_eq!(llm_runtime_summary.generated_action_count, 2);
        ...
        let coverage = row
            ...
            .as_ref()
            .expect("llm surface coverage");
        assert_eq!(coverage.overall_status, "progress_observed");
        assert_eq!(
    ...
    #[test]
    fn recent_sim_run_history_projects_llm_runtime_receipts_and_categories_surface_coverage() {
        let store = MockStore::new();
        let now = now_ts();
        let run_started_at = now.saturating_sub(90);

        let persist_llm_runtime_record = |ts: u64, path: &str, status: u16| {
            persist_event_record(
                &store,
                EventLogRecord {
                    entry: EventLogEntry {
                    ...
                    outcome_code: None,
                    botness_score: None,
                    sim_run_id: Some("simrun-llm-runtime-coverage-proof".to_string()),
                    sim_profile: Some("llm_runtime_lane.request_mode".to_string()),
                    sim_lane: Some("bot_red_team".to_string()),
                    is_simulation: true,
                    scrapling_surface_receipts: Vec::new(),
                    ...
                    llm_runtime_summary: Some(
                        crate::admin::adversary_sim::LlmRuntimeRecentRunSummary {
                            ...
                            passed_tick_count: 1,
                            failed_tick_count: 0,
                            last_response_status: Some(status),
                            failure_class: None,
                            error: None,
                            terminal_failure: None,
                            latest_realism_receipt: Some(
                                crate::admin::adversary_sim_worker_plan::LlmRuntimeRealismReceipt {
                                    ...
                                    transport_emission_basis: "python_urllib_runtime"
                                        .to_string(),
                                    transport_degraded_reason:
                                        "no_tls_or_protocol_impersonation_support".to_string(),
                                    observed_user_agent_families: vec![
                                        "chrome_android".to_string(),
                                    ],
                                    observed_accept_languages: vec![
        ...
        persist_llm_runtime_record(run_started_at, "/", 200);
        persist_llm_runtime_record(run_started_at.saturating_add(1), "/challenge", 403);

        let recent_runs = monitoring_recent_sim_run_summaries(&store, now, 24, 10);
        let row = recent_runs
            .iter()
            .find(|value| value.run_id == "simrun-llm-runtime-coverage-proof")
            .expect("llm runtime coverage row");
        let coverage = row
            .llm_surface_coverage
            .as_ref()
            .expect("llm surface coverage");
        assert_eq!(row.monitoring_event_count, 0);
        assert_eq!(row.defense_delta_count, 0);
        ...
        assert_eq!(
            coverage
                .surface_labels
                .get("challenge_routing")
                .map(String::as_str),
            Some("Challenge Routing")
        );
        assert_eq!(coverage.receipts.len(), 2);
    }
...
mod admin_config_tests {
    ...
    #[test]
    fn post_sim_oversight_route_archives_strict_human_only_profile_context() {
        let _lock = crate::test_support::lock_env();
        std::env::set_var("SHUMA_ADMIN_CONFIG_WRITE_ENABLED", "true");
        std::env::set_var("SHUMA_RUNTIME_ENV", "runtime-dev");
        std::env::set_var("SHUMA_ADVERSARY_SIM_AVAILABLE", "true");
        std::env::set_var("SHUMA_GATEWAY_DEPLOYMENT_PROFILE", "shared-server");
        std::env::set_var("SHUMA_API_KEY", "oversight-human-only-proof-test-key");
        std::env::set_var("SHUMA_FORWARDED_IP_SECRET", "test-forwarded-secret");
        ...
        assert_eq!(
            snapshot_json["objectives"]["profile_id"].as_str(),
            Some("human_only_private")
        );
        assert_eq!(
            snapshot_json["verified_identity"]["effective_non_human_policy"]
                ["verified_identity_override_mode"]
                .as_str(),
            Some("strict_human_only")
        );

        let post_sim_req = make_internal_oversight_request(
            "oversight-human-only-proof-test-key",
            serde_json::to_vec(&serde_json::json!({
                "trigger_kind": "post_adversary_sim",
                "sim_run_id": "simrun-human-only-proof-001",
                "sim_completion_reason": "auto_window_expired"
            }))
            .expect("json body")
            .as_slice(),
        ...
        let history_json: serde_json::Value =
            serde_json::from_slice(history_resp.body()).expect("history decodes");
        assert_eq!(
            history_json["episode_archive"]["rows"][0]["evaluation_context"]["profile_id"]
                .as_str(),
            Some("human_only_private")
        );
        assert!(matches!(
    ...
    #[test]
    fn post_sim_oversight_route_records_repeated_retained_and_rolled_back_episodes_against_changed_configs(
    ) {
        let _lock = crate::test_support::lock_env();
        std::env::set_var("SHUMA_ADMIN_CONFIG_WRITE_ENABLED", "true");
        std::env::set_var("SHUMA_RUNTIME_ENV", "runtime-dev");
        ...
        let first_post_sim_json: serde_json::Value =
            serde_json::from_slice(first_post_sim_resp.body()).expect("first post-sim decodes");
        assert_eq!(
            first_post_sim_json["run"]["execution"]["apply"]["stage"].as_str(),
            Some("canary_applied")
        );
        assert_eq!(
            first_post_sim_json["run"]["execution"]["apply"]["patch_family"].as_str(),
        ...
        let second_post_sim_json: serde_json::Value =
            serde_json::from_slice(second_post_sim_resp.body()).expect("second post-sim decodes");
        assert_eq!(
            second_post_sim_json["run"]["execution"]["apply"]["stage"].as_str(),
            Some("canary_applied")
        );
        assert_eq!(
        ...
        let mut second_active_canary: serde_json::Value = serde_json::from_slice(
            &store
                .get("oversight_active_canary:v1:default")
                .expect("second active canary lookup")
                .expect("second active canary present"),
        )
        .expect("second active canary decodes");
        second_active_canary["opened_at_ts"] = serde_json::json!(1);
        second_active_canary["watch_window_end_at"] = serde_json::json!(1);
        store
            .set(
                "oversight_active_canary:v1:default",
                &serde_json::to_vec(&second_active_canary)
                    .expect("second active canary encodes"),
            )
            .expect("second active canary update");
        ...
        let status_rows = status_json["episode_archive"]["rows"]
            .as_array()
            .expect("status episode archive rows");
        assert_eq!(status_rows.len(), 2);
        assert!(status_rows.iter().any(|row| {
            row["retain_or_rollback"].as_str() == Some("rolled_back")
                && row["proposal"]["patch_family"].as_str() == Some("cdp_detection")
        }));
        assert!(status_rows.iter().any(|row| {
            row["retain_or_rollback"].as_str() == Some("retained")
                && row["proposal"]["patch_family"].as_str() == Some("fingerprint_signal")
        }));
        ...
        let history_json: serde_json::Value =
            serde_json::from_slice(history_resp.body()).expect("history decodes");
        let history_rows = history_json["episode_archive"]["rows"]
            .as_array()
            .expect("history episode archive rows");
...
struct MonitoringRecentSimRunAccumulator {
    ...
    llm_surface_observations:
        Vec<crate::observability::llm_surface_observation::LlmSurfaceObservationRow>,
    latest_scrapling_realism_receipt:
...
fn monitoring_recent_sim_run_summaries_filtered<S: crate::challenge::KeyValueStore>(
    ...
    for stored in load_recent_event_records_with_keys(store, now, hours) {
        ...
        if let Some(summary) = stored.record.llm_runtime_summary.as_ref() {
            accumulator.llm_surface_observations.extend(
                crate::observability::llm_surface_observation::summarize_llm_surface_observations(
                    Some(summary),
                ),
            );
