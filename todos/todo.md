...
- [ ] TEST-ENV-1 Enforce repo-wide Rust test env-isolation discipline so any test that mutates process env must hold `lock_env()`. The named offender in `src/runtime/shadow_mode/tests.rs` is now fixed; the remaining work is the broader explicit repo-wide audit and cleanup.
