...
- The next live RSI blocker is now post-canary candidate-window materialization rather than cadence: without a second protected post-change Scrapling window, terminal judgment fail-closes as `candidate_window_not_materialized`; with one, the same loop reaches a meaningful measured terminal outcome.
