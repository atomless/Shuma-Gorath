...
- A fresh regression review has now reopened the next active adversary-realism blocker: on the current generated root-hosted site, request-native Scrapling can overclaim hostile surface coverage from ordinary clean `200` responses, remains biased toward the old dummy-site path shape, and rotates modes too slowly because one serialized five-mode cycle can now stretch across roughly `104s`. The next execution priority is therefore `SIM-REALISM-REG-1A..1C`, not proxy-environment readiness.
...
- `SIM-REALISM-REG-1` is now landed: root-started Scrapling on the generated root-hosted site now proves policy-served defence confrontation from ordinary public entrypoints, request-native and browser personas follow those Shuma-served interstitials without fake route hints, aggressive request-native pressure can honestly be escalated into tarpit on the real defence path, and serialized Scrapling mode turnover is bursty again instead of stretching into the earlier roughly `104s` watch-window collapse. The next active execution priority is now `SIM-REALISM-ENV-1A`.
