...
- Architecture gate:
  - Settle the prerequisite decisions in [`docs/plans/2026-03-18-monitoring-telemetry-foundations-architectural-necessities.md`](../docs/plans/2026-03-18-monitoring-telemetry-foundations-architectural-necessities.md) before implementing the counter, summary, and read-surface parts of this tranche.
- [ ] MON-TEL-1-1 Define the canonical coarse traffic-lane taxonomy and denominator contract for Monitoring, including current-runtime lanes, reserved future verified-identity lanes, exactness labels, and the minimal route/action-family breakdown that stays within current cardinality guardrails. Use [`docs/plans/2026-03-18-monitoring-traffic-lane-and-denominator-contract.md`](../docs/plans/2026-03-18-monitoring-traffic-lane-and-denominator-contract.md) as the design contract, including its grouped current-runtime branch mapping guardrail and explicit static-bypass accounting decision.
- [ ] MON-TEL-1-2 Add bounded coarse counters for total requests seen, forwarded requests, short-circuited requests, and response-byte totals by lane and execution mode, using the existing shared observability patterns rather than widening event rows or adding a parallel analytics path. Use [`docs/plans/2026-03-18-monitoring-request-outcome-telemetry-hook-contract.md`](../docs/plans/2026-03-18-monitoring-request-outcome-telemetry-hook-contract.md) as the design contract for one authoritative final-outcome hook, including the buffered-emission rule and strict single-emission invariant.
- [ ] MON-TEL-1-3 Add the minimum new denominator counters needed to turn existing human-friction telemetry into rates instead of raw counts, including likely-human or interactive denominators where Shuma can support them truthfully without pretending to know more than it does.
- [ ] MON-TEL-1-4 Define and implement one normalized defence-effectiveness funnel contract reused across defence families where the data is available, so operators can compare candidate, triggered, escalated, denied, suspicious-forwarded, and likely-human-affected traffic in one shared shape instead of per-module bespoke summaries. Reuse the outcome taxonomy from [`docs/plans/2026-03-18-monitoring-request-outcome-telemetry-hook-contract.md`](../docs/plans/2026-03-18-monitoring-request-outcome-telemetry-hook-contract.md) instead of introducing family-specific outcome shapes.
- [ ] MON-TEL-1-5 Materialize bounded hot-read summaries for lane mix, suspicious-cost posture, and normalized defence funnels, with explicit freshness, size, exactness, and ownership-tier contracts that preserve the current bootstrap and delta budget discipline on shared-host and Fermyon. Use [`docs/plans/2026-03-18-monitoring-operator-summary-exactness-contract.md`](../docs/plans/2026-03-18-monitoring-operator-summary-exactness-contract.md) and [`docs/plans/2026-03-18-monitoring-bootstrap-and-supporting-summary-ownership-contract.md`](../docs/plans/2026-03-18-monitoring-bootstrap-and-supporting-summary-ownership-contract.md) as the design contracts.
