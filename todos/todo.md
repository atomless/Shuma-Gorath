...
- `SIM-REALISM-2I` Task 1 is now landed: the current client-IP collapse matrix is executable through `make test-client-ip-topology-contract`, with focused proof for shared-host direct traffic, trusted proxy extraction, misconfigured proxy fail-closed behavior, edge `true-client-ip`, edge trusted-header fallback, edge degraded fallback, and `/shuma/health` single-hop versus multi-hop rules. The remaining active work in `SIM-REALISM-2I` is now the trusted sim-ingress path itself.
