...
- [ ] SIM-SCR-LANE-1 Three-lane runtime migration and Scrapling integration.
  ...
  - [ ] SIM-SCR-8 Close operator workflow, truthful `make` verification targets, rollout/rollback guidance, hosted-scope deployment egress guidance, tranche evidence, and removal of remaining catalog-first runtime directives so "telemetry is the map" stays explicit in operator docs and plan language.
    ...
    - [ ] SIM-SCR-8-1 Add one shared Scrapling deploy-preflight helper plus durable `.shuma/` receipt that infers scope, root-only primary seed, supervisor env wiring, verification commands, and deployment-egress guidance from deploy context instead of requiring hand-authored operator artifacts.
    - [ ] SIM-SCR-8-2 Integrate the shared-host deploy path and normalized `ssh_systemd` remote receipts so shared-host deployment uploads Scrapling scope-and-seed artifacts, persists `ADVERSARY_SIM_SCRAPLING_*` env values automatically, and keeps that behavior working through `make remote-update`.
