...
- [ ] MON-TEL-1-2A Extend the landed origin-aware request-outcome counter family with forwarded-versus-local byte attribution by bounded outcome semantics at scope and lane level, preserving the existing buffered observability path and avoiding any new analytics subsystem.
- [ ] MON-TEL-1-2B Bring the remaining control/fail-path terminal outcomes under the request-outcome contract where feasible, and explicitly codify and prove any intentional exclusions that must remain outside it.
...
- [ ] MON-TEL-1-5A Materialize bounded hot-read summary rows for `response_kind` that are rich enough for benchmark analysis but still safe for current hot-read budget and ownership constraints.
- [ ] MON-TEL-1-5B Materialize bounded hot-read summary rows for `policy_source` that expose first-tranche versus second-tranche routing truth without reverting to bespoke per-module monitoring summaries.
- [ ] MON-TEL-1-5C Materialize bounded hot-read summary rows for `route_action_family` so route-family leakage and effectiveness can later be analyzed without raw-counter spelunking.
