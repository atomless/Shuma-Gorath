...
- [ ] TEST-TIER-1 Define and document canonical test tiers and truthful target scope across `Makefile` and docs: separate static/source-contract checks, local behavior tests, Spin runtime integration tests, rendered dashboard tests, and live operational proofs; refresh stale descriptions such as the integration scenario count and make the live shared-host proof tier explicit rather than implicit.
- [ ] TEST-HYGIENE-6 Replace remaining non-behavior source archaeology outside explicit contract lanes, starting with dashboard Node source-regex tests, supervisor wrapper tests, integration cleanup shell-shape checks, and feature-specific Makefile selector microtests; where source-shape checks remain necessary, reclassify them as explicit `contract` or `wiring` proof instead of feature behavior proof.
