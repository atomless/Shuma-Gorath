...
- [ ] TUNE-SURFACE-1 Complete the Tuning tab and related config-control surfaces as the full operator contract for route, defence, ban, recidive, and intelligence thresholds.
  Blocker: defer execution until the operator monitoring overhaul defines which knobs are actionable, which remain diagnostic, and which are safe for future controller tuning. The 2026-03-21 feedback-loop review also makes `ARCH-OBS-1`, `OPS-BENCH-2`, and `OPS-SNAPSHOT-2` explicit prerequisites so Tuning does not invent operator semantics before the missing benchmark/objective/decision-lineage truth is delivered.
