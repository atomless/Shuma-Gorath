...
- [ ] OVR-APPLY-1 First closed autonomous tuning loop with bounded canary apply, watch window, benchmark judgment, and rollback.
  Blocker: defer execution until `TRAFFIC-TAX-1`, `TRAFFIC-TAX-2`, `SIM-PROTECTED-1`, `SIM-COVER-1`, `OPS-OBJECTIVES-3`, and `OPS-BENCH-3` land and prove that auto-apply will operate only on protected, category-aware evidence. `synthetic_traffic` must remain tuning-ineligible, raw frontier or LLM discoveries must remain advisory until replay promotion or equivalent deterministic confirmation makes them protected evidence, and lane representativeness must be measured against Shuma's own observed traffic taxonomy rather than lane-local assumptions.
