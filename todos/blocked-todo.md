...
- [ ] TUNE-SURFACE-1 Complete the Tuning tab and related config-control surfaces as the full operator contract for route, defence, ban, recidive, and intelligence thresholds.
  Blocker: defer execution until `ARCH-OBS-1`, `OPS-BENCH-2`, `OPS-SNAPSHOT-2`, `OVR-RECON-1`, and `OVR-AGENT-1` define the recommend-only backbone, and then additionally wait for `TRAFFIC-TAX-1`, `TRAFFIC-TAX-2`, `SIM-FULFILL-1`, `SIM-COVER-1`, `SIM-PROTECTED-1`, `OPS-OBJECTIVES-3`, `OPS-BENCH-3`, and the delivered live-proven `OVR-APPLY-1` loop per [`../docs/research/2026-03-22-ovr-apply-1-canary-apply-and-rollback-post-implementation-review.md`](../docs/research/2026-03-22-ovr-apply-1-canary-apply-and-rollback-post-implementation-review.md). Also wait for `ADV-DIAG-1` so Tuning does not project adversary-sim status counters that diverge from the persisted event truth. Tuning must expose proven patch families, protected-evidence rules, category-aware objectives, taxonomy-backed lane coverage, stable operator-facing taxonomy labels, the bounded posture scale from `allowed` through `blocked`, classification-confidence lineage, and rollback semantics rather than speculate ahead of the backend loop.
