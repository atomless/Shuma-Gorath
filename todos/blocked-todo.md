...
- [ ] OVR-APPLY-1 First closed autonomous tuning loop with bounded canary apply, watch window, benchmark judgment, and rollback.
  Blocker: defer execution until `TRAFFIC-TAX-1`, `TRAFFIC-TAX-2`, `SIM-FULFILL-1`, `SIM-PROTECTED-1`, `SIM-COVER-1`, `OPS-OBJECTIVES-3`, and `OPS-BENCH-3` land and prove that auto-apply will operate only on protected, category-aware evidence. `synthetic_traffic` must remain tuning-ineligible, raw frontier or LLM discoveries must remain advisory until replay promotion or equivalent deterministic confirmation makes them protected evidence, and lane representativeness must be measured against Shuma's canonical taxonomy rather than lane-local assumptions.
...
- [ ] TUNE-SURFACE-1 Complete the Tuning tab and related config-control surfaces as the full operator contract for route, defence, ban, recidive, and intelligence thresholds.
  Blocker: defer execution until `ARCH-OBS-1`, `OPS-BENCH-2`, `OPS-SNAPSHOT-2`, `OVR-RECON-1`, and `OVR-AGENT-1` define the recommend-only backbone, and then additionally wait for `TRAFFIC-TAX-1`, `TRAFFIC-TAX-2`, `SIM-FULFILL-1`, `SIM-PROTECTED-1`, `SIM-COVER-1`, `OPS-OBJECTIVES-3`, `OPS-BENCH-3`, and `OVR-APPLY-1` to prove the closed-loop tuning semantics. Tuning must expose proven patch families, protected-evidence rules, category-aware objectives, taxonomy-backed lane coverage, and rollback semantics rather than speculate ahead of the backend loop.
