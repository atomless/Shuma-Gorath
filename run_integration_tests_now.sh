...
echo -e "${YELLOW}  Running 5 test scenarios${NC}"
echo -e "${YELLOW}============================================${NC}"
echo ""
...
curl -s -H "X-Forwarded-For: 1.2.3.4" "$BASE_URL/bot-trap" > /dev/null
sleep 0.5
resp=$(curl -s -H "X-Forwarded-For: 1.2.3.4" "$BASE_URL/")
...
ban_resp=$(curl -s -X POST -H "Authorization: Bearer $API_KEY" \
