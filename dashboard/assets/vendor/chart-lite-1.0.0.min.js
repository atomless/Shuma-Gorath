...
(function (global) {
  ...
  function drawDoughnut(ctx, size, data, style) {
    const values = asArray(data).map(asNumber).filter((value) => value > 0);
    if (values.length === 0) {
    ...
    const outer = Math.max(20, Math.floor(Math.min(size.width, size.height) * 0.35));
  ...
  function drawBars(ctx, size, values, style) {
    const max = Math.max(1, ...values);
    const axisLeft = 30;
    const axisBottom = size.height - 14;
    ...
    ctx.save();
    ctx.strokeStyle = 'rgba(86, 86, 86, 0.25)';
    ctx.lineWidth = 1;
    ctx.beginPath();
  ...
  function drawLine(ctx, size, values, style) {
    const max = Math.max(1, ...values);
    const left = 16;
    const top = 12;
    const right = size.width - 10;
    const bottom = size.height - 16;
  ...
  ChartLite.prototype.update = function update() {
    ...
    if (this.type === 'doughnut') {
      drawDoughnut(this.ctx, size, values, style);
      return;
    }
    if (this.type === 'line') {
      drawLine(this.ctx, size, values, asArray(style));
      return;
    }
    drawBars(this.ctx, size, values, asArray(style));
  };
